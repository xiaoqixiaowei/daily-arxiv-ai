...
### [1] [UESF-Bench: Benchmarking and Probing for Unified Embodied Seeking and Following](https://arxiv.org/abs/2607.13621)
...
TL;DR: 本文围绕《UESF-Bench: Benchmarking and Probing for Unified Embodied Seeking and Following》展开，核心内容是：Language-guided human following is an important capability for embodied agents, but existing benchmarks typically assume that the target person is visible at the start of an episode. This setting simplifies the problem and overlooks a more realistic requirement: an agent often needs to first find a language-described target and then persistently follow that target in a dynamic environment.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：This setting simplifies the problem and overlooks a more realistic requirement: an agent often needs to first find a language-described target and then persistently follow that target in a dynamic environment.

Method: 方法上，论文主要采用或提出了如下思路：Language-guided human following is an important capability for embodied agents, but existing benchmarks typically assume that the target person is visible at the start of an episode.

Result: 实验或分析结果显示：While recent work has started to study human search, existing settings are typically evaluated in task-specific scenarios and often rely on stronger prior knowledge of the environment.

Conclusion: 总体来看，论文结论是：Experimental results show that SeekFollow-VLA achieves clear improvements over both single-head and dual-head baselines across single-person and multi-person environments, establishing a baseline for unified embodied seek-and-follow.
...
TL;DR: 本文围绕《HRIBench: Benchmarking Interaction-Centric Human-Robot Collaboration》展开，核心内容是：Current vision-language-action (VLA) benchmarks primarily evaluate isolated manipulation skills while leaving human-robot interaction structure largely unmodeled. However, real-world collaboration fundamentally requires coordination under shared agency, including intent understanding, temporal synchronization, protocol adherence, and safe interaction in dynamic environments.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：To address this gap, we introduce HRIBench, a diagnostic benchmark for intent-aware human-robot collaboration based on executable interaction scenarios.

Method: 方法上，论文主要采用或提出了如下思路：Current vision-language-action (VLA) benchmarks primarily evaluate isolated manipulation skills while leaving human-robot interaction structure largely unmodeled.

Result: 实验或分析结果显示：Current vision-language-action (VLA) benchmarks primarily evaluate isolated manipulation skills while leaving human-robot interaction structure largely unmodeled.

Conclusion: 总体来看，论文结论是：43, demonstrating the benchmark's value for advancing interaction-centric robot learning.
...
### [3] [HRO: Hierarchical Room-to-Object Framework for Zero-Shot Object Goal Navigation with Large Language Models](https://arxiv.org/abs/2607.13072)
...
TL;DR: 本文围绕《HRO: Hierarchical Room-to-Object Framework for Zero-Shot Object Goal Navigation with Large Language Models》展开，核心内容是：Zero-shot object-goal navigation aims to enable an intelligent agent to explore and navigate to objects of unknown categories in an unfamiliar environment without specific target training. In zero-shot navigation tasks, pre-trained large models are usually employed to leverage their prior knowledge for guiding the agent's navigation.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：They lack the hierarchical spatial cognition modeling of human-like room semantics to object localization, which leads to strong blindness in exploration, insufficient accuracy in semantic association, and failure to fully unleash the common-sense reasoning potential of LLMs.

Method: 方法上，论文主要采用或提出了如下思路：This paper proposes an LLM-driven hierarchical room-to-object (HRO) framework for zero-shot object-goal navigation, which guides the agent to explore and navigate to the target object in a coarse-to-fine manner.

Result: 实验或分析结果显示：Experiments on Gibson and HM3D datasets verify that our HRO framework achieves superior success rate and generalization over existing LLM-based methods, underscoring LLMs' strong potential for zero-shot object-goal navigation.

Conclusion: 总体来看，论文结论是：Experiments on Gibson and HM3D datasets verify that our HRO framework achieves superior success rate and generalization over existing LLM-based methods, underscoring LLMs' strong potential for zero-shot object-goal navigation.
...
### [4] [Generalizable VLA Finetuning via Representation Anchoring and Language-Action Alignment](https://arxiv.org/abs/2607.13429)
...
TL;DR: 本文围绕《Generalizable VLA Finetuning via Representation Anchoring and Language-Action Alignment》展开，核心内容是：Finetuning a pretrained vision-language model (VLM) on robot demonstrations via behavior cloning (BC) has become the standard recipe for vision-language-action (VLA) policies. However, BC finetuning progressively overwrites the pretrained representations that support visual and semantic generalization.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Finetuning a pretrained vision-language model (VLM) on robot demonstrations via behavior cloning (BC) has become the standard recipe for vision-language-action (VLA) policies.

Method: 方法上，论文主要采用或提出了如下思路：However, BC finetuning progressively overwrites the pretrained representations that support visual and semantic generalization.

Result: 实验或分析结果显示：On a physical xArm7 robot, across two widely used VLA architectures, Anchor-Align improves real-robot success on both (28% to 54% and 37% to 60%).
...
### [5] [Joint On-and-Off Policy Learning for Vision-and-Language Navigation](https://arxiv.org/abs/2607.13461)
...
TL;DR: 本文围绕《Joint On-and-Off Policy Learning for Vision-and-Language Navigation》展开，核心内容是：Vision-and-Language Navigation (VLN) necessitates an embodied agent to navigate in the physical world by adhering to natural language instructions. Recent advancements in Vision-Language Models (VLM) have propelled the development of VLM-based VLN methods with two predominant paradigms: (1) imitation learning (IL) on expert demonstrations, followed by the Dataset Aggregation (DAgger) algorithm to bolster error recovery capabilities; (2) reinforcement learning (RL) driven by verifiable rewards to enhance reasoning and exploration.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：A notable gap is the absence of integration between these two distinct paradigms.

Method: 方法上，论文主要采用或提出了如下思路：Recent advancements in Vision-Language Models (VLM) have propelled the development of VLM-based VLN methods with two predominant paradigms: (1) imitation learning (IL) on expert demonstrations, followed by the Dataset Aggregation (DAgger) algorithm to bolster error recovery capabilities; (2) reinforcement learning (RL) driven by verifiable rewards to enhance reasoning and exploration.

Result: 实验或分析结果显示：Subsequently, the DAgger algorithm is utilized to generate heuristic exploration trajectories, which are then used for imitation learning to improve error recovery capabilities.
...
### [6] [Active Trust Management for Successful Human-Robot Teaming: Moving from a Trust Repair to a Trust Satisficing Perspective](https://arxiv.org/abs/2607.13595)
...
TL;DR: 本文围绕《Active Trust Management for Successful Human-Robot Teaming: Moving from a Trust Repair to a Trust Satisficing Perspective》展开，核心内容是：Integrating mobile robots into human teams promises significant capability improvements for tasks such as searching hazardous environments. Unlike existing teleoperated robots, future robot systems will increasingly be endowed with some level of artificial intelligence (AI), giving them a degree of autonomy in how they pursue mission goals.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Integrating mobile robots into human teams promises significant capability improvements for tasks such as searching hazardous environments.

Method: 方法上，论文主要采用或提出了如下思路：Unlike existing teleoperated robots, future robot systems will increasingly be endowed with some level of artificial intelligence (AI), giving them a degree of autonomy in how they pursue mission goals.

Result: 实验或分析结果显示：Integrating mobile robots into human teams promises significant capability improvements for tasks such as searching hazardous environments.

Conclusion: 总体来看，论文结论是：We refer to a recent experimental exploration of 'swift trust' and a novel behavioral trust metric for HRT, and we highlight issues for further investigation.
...
TL;DR: 本文围绕《Just-In-Time Scene Graph Growth: Combating Perceptual Saturation in Long-Horizon Robotics》展开，核心内容是：While 3D Scene Graphs (3DSGs) provide crucial structured representations for embodied agents, conventional Ahead-of-Time, build-everything-then-filter pipelines conflict with the real-time, low-latency demands of edge platforms, inducing a perceptual saturation effect via severe observation redundancy. To resolve this, we present JITOMA (Just-In-Time On-demand Memory Activation), a closed-loop framework that unifies task reasoning, perception, and memory into a just-in-time growth process.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：While 3D Scene Graphs (3DSGs) provide crucial structured representations for embodied agents, conventional Ahead-of-Time, build-everything-then-filter pipelines conflict with the real-time, low-latency demands of edge platforms, inducing a perceptual saturation effect via severe observation redundancy.

Method: 方法上，论文主要采用或提出了如下思路：While 3D Scene Graphs (3DSGs) provide crucial structured representations for embodied agents, conventional Ahead-of-Time, build-everything-then-filter pipelines conflict with the real-time, low-latency demands of edge platforms, inducing a perceptual saturation effect via severe observation redundancy.

Result: 实验或分析结果显示：To evaluate these dynamic capabilities and study perceptual saturation trade-offs, we introduce JITOMA-Bench, a comprehensive suite for long-horizon multi-tasking and complex multi-step reasoning.

Conclusion: 总体来看，论文结论是：Extensive experiments demonstrate that JITOMA substantially reduces active graph size and captioning latency, while maintaining stable processing time under long-horizon task switching.
...
### [8] [Semantic Anchoring for Robotic Action Representations](https://arxiv.org/abs/2607.13597)
...
TL;DR: 本文围绕《Semantic Anchoring for Robotic Action Representations》展开，核心内容是：Vision-Language-Action (VLA) models inherit rich semantic representations from pretrained Vision-Language Models, yet fine-tuning on limited robot demonstrations degrades this structure and undermines generalization. A fundamental question therefore arises: what constitutes a good action representation?


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Vision-Language-Action (VLA) models inherit rich semantic representations from pretrained Vision-Language Models, yet fine-tuning on limited robot demonstrations degrades this structure and undermines generalization.

Method: 方法上，论文主要采用或提出了如下思路：Vision-Language-Action (VLA) models inherit rich semantic representations from pretrained Vision-Language Models, yet fine-tuning on limited robot demonstrations degrades this structure and undermines generalization.

Result: 实验或分析结果显示：Validated on different VLA backbones across simulation and real-world benchmarks, our method yields up to +18.

Conclusion: 总体来看，论文结论是：5% on out-of-distribution generalization.
...
### [9] [An Empirical Study on Stage-Information Interfaces for VLA Fine-Tuning](https://arxiv.org/abs/2607.13605)
*Yingwei Ji*
...
TL;DR: 本文围绕《An Empirical Study on Stage-Information Interfaces for VLA Fine-Tuning》展开，核心内容是：One high-level instruction in long-horizon manipulation can cover several action stages. We use segmented action annotations as an intermediate representation between the full-task instruction and VLA action chunks.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：One high-level instruction in long-horizon manipulation can cover several action stages.

Method: 方法上，论文主要采用或提出了如下思路：We use segmented action annotations as an intermediate representation between the full-task instruction and VLA action chunks.

Result: 实验或分析结果显示：Under direct fine-tuning, full-task instruction, current-stage text, and Ordinal Stage-State achieve mean success rates of 57.

Conclusion: 总体来看，论文结论是：The observed benefit differs across interface representations and training arrangements.
...
### [10] [Learning Robust Execution in Robotic Manipulation with Agentic Reinforcement Learning](https://arxiv.org/abs/2607.13818)
...
TL;DR: 本文围绕《Learning Robust Execution in Robotic Manipulation with Agentic Reinforcement Learning》展开，核心内容是：Robotic manipulation poses fundamental challenges due to uncertainty, long-horizon execution, and compounding errors, which can easily destabilize execution and lead to task failure. Although recent vision-language-action (VLA) models exhibit strong generalization, they typically lack explicit mechanisms to assess execution stability and to recover when execution deviates from its nominal behavior.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Robotic manipulation poses fundamental challenges due to uncertainty, long-horizon execution, and compounding errors, which can easily destabilize execution and lead to task failure.

Method: 方法上，论文主要采用或提出了如下思路：In this paper, we propose: (1) two complementary metrics to assess execution quality at runtime, and (2) an agentic reinforcement learning framework that learns to restore effective execution through high-level decision-making rather than directly learning low-level actions.

Result: 实验或分析结果显示：We evaluate the proposed method on the LIBERO benchmark, achieving up to a 13.

Conclusion: 总体来看，论文结论是：2% improvement under disturbance settings, demonstrating substantially enhanced execution robustness.
...
### [11] [S-squared-VLA: Decoupling Semantic and Spatial Streams in Vision-Language-Action Models for Autonomous Driving](https://arxiv.org/abs/2607.13926)
...
TL;DR: 本文围绕《S-squared-VLA: Decoupling Semantic and Spatial Streams in Vision-Language-Action Models for Autonomous Driving》展开，核心内容是：Vision-Language Models (VLMs) have demonstrated remarkable potential for high-level reasoning in autonomous driving, yet they fundamentally struggle to generate precise, low-level control actions. This limitation is rooted in a semantic-physical gap caused by the inherent mismatch between discrete language tokens and continuous trajectory planning.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：This limitation is rooted in a semantic-physical gap caused by the inherent mismatch between discrete language tokens and continuous trajectory planning.

Method: 方法上，论文主要采用或提出了如下思路：Standard VLAs experience a severe spatial representation collapse, which irreversibly degrades the fine-grained spatial and geometric priors essential for safe, boundary-aware navigation.

Result: 实验或分析结果显示：Evaluations on the NAVSIM closed-loop benchmark show that S-squared-VLA achieves a Predictive Driver Model Score (PDMS) of 87.

Conclusion: 总体来看，论文结论是：4 among all evaluated methods.
...
### [12] [PhysClaw-0: A Symbiotic Agentic System for Robot Autonomy via Language Corrections](https://arxiv.org/abs/2607.14047)
...
TL;DR: 本文围绕《PhysClaw-0: A Symbiotic Agentic System for Robot Autonomy via Language Corrections》展开，核心内容是：Autonomous data collection governs the volume and quality of real-world trajectories for manipulation policy learning. Existing pipelines reduce human effort via self-resetting, VLM verification, or language-guided correction, yet episode-scoped fixes must be reissued whenever the same failure recurs, so oversight cost grows with session length rather than with the number of distinct problems.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Existing pipelines reduce human effort via self-resetting, VLM verification, or language-guided correction, yet episode-scoped fixes must be reissued whenever the same failure recurs, so oversight cost grows with session length rather than with the number of distinct problems.

Method: 方法上，论文主要采用或提出了如下思路：We present PhysClaw-0, a human-robot symbiotic agentic system in which corrections are retained and reused across rounds.

Result: 实验或分析结果显示：Language corrections improve verifier-human agreement in all four evaluated settings and raise average single-attempt success from 12.

Conclusion: 总体来看，论文结论是：Policies fine-tuned on PhysClaw-0 data match teleoperation-trained policy success at a fraction of collection human cost.
...
### [13] [Exploratory, Communicative, and Deployable: Vision-Driven Embodied Agents for Open-World Mobile Manipulation](https://arxiv.org/abs/2607.13653)
...
TL;DR: 本文围绕《Exploratory, Communicative, and Deployable: Vision-Driven Embodied Agents for Open-World Mobile Manipulation》展开，核心内容是：Real-world deployment of embodied agents requires active exploration, visual grounding, and interactive intent disambiguation. However, existing frameworks often rely on privileged simulator states or assume complete instructions, bypassing realistic deployment challenges.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：However, existing frameworks often rely on privileged simulator states or assume complete instructions, bypassing realistic deployment challenges.

Method: 方法上，论文主要采用或提出了如下思路：However, existing frameworks often rely on privileged simulator states or assume complete instructions, bypassing realistic deployment challenges.

Result: 实验或分析结果显示：To comprehensively evaluate this approach, we introduce REAL-Bench, a benchmark spanning 241 tasks across active exploration, visual distraction, articulated manipulation, and interactive disambiguation.
...
### [14] [M$^\text{4}$World: A Multi-view Multimodal Driving World Model for Interactive Object Manipulation and Minute-long Streaming](https://arxiv.org/abs/2607.14005)
...
TL;DR: 本文围绕《M$^\text{4}$World: A Multi-view Multimodal Driving World Model for Interactive Object Manipulation and Minute-long Streaming》展开，核心内容是：Driving-world generation has emerged as a core capability for scalable autonomous-driving simulation, yet existing methods remain limited in object-level controllability and long-horizon stability. We present M$^\text{4}$World, a Multi-view and Multimodal generative driving world model that synthesizes future surround-view video streams and synchronized LiDAR scans while supporting interactive object Manipulation and stable Minute-long streaming.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Driving-world generation has emerged as a core capability for scalable autonomous-driving simulation, yet existing methods remain limited in object-level controllability and long-horizon stability.

Method: 方法上，论文主要采用或提出了如下思路：We present M$^\text{4}$World, a Multi-view and Multimodal generative driving world model that synthesizes future surround-view video streams and synchronized LiDAR scans while supporting interactive object Manipulation and stable Minute-long streaming.

Result: 实验或分析结果显示：Stable minute-long streaming, on the other hand, is achieved through a multi-stage training framework that enables online causal generation in only four denoising steps while maintaining coherent world dynamics throughout extended rollouts.

Conclusion: 总体来看，论文结论是：Together with downstream long-tail augmentation and scene editing, these results demonstrate the potential of M$^\text{4}$World for controllable, scalable driving simulation.
...
### [15] [ThinkBLOX: 3D Indoor Scene Generation with Progressive Reasoning](https://arxiv.org/abs/2607.13539)
...
TL;DR: 本文围绕《ThinkBLOX: 3D Indoor Scene Generation with Progressive Reasoning》展开，核心内容是：While traditional graphics methods often synthesize 3D indoor scenes autoregressively or hierarchically, recent vision-language model (VLM)-based generators predominantly adopt a one-shot paradigm where the full layout is planned at once. This one-shot approach often requires global re-optimization or complete reconstruction during interactive editing (e.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：To address these challenges, we propose ThinkBLOX, a VLM-based progressive reasoning framework that iteratively designs and refines 3D scenes.

Method: 方法上，论文主要采用或提出了如下思路：To address these challenges, we propose ThinkBLOX, a VLM-based progressive reasoning framework that iteratively designs and refines 3D scenes.

Result: 实验或分析结果显示：Extensive experiments show that ThinkBLOX significantly outperforms recent one-shot and iterative baselines in physical plausibility, semantic alignment, and interactive editability.

Conclusion: 总体来看，论文结论是：Additionally, we show that it supports diverse applications, including both global and local generation and rearrangement of 3D scenes.
...
### [16] [Designing Safety-Constrained LLM Systems for Public Health Information Access](https://arxiv.org/abs/2607.13038)
...
TL;DR: 本文围绕《Designing Safety-Constrained LLM Systems for Public Health Information Access》展开，核心内容是：We present the design and implementation of a safety constrained large language model (LLM) system for public health information access, focusing on maternal and child health (MCH) resource navigation. While LLM based systems offer flexible and natural interfaces for information retrieval, their deployment in healthcare contexts introduces risks related to safety, trust, and uncontrolled generation.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：We present the design and implementation of a safety constrained large language model (LLM) system for public health information access, focusing on maternal and child health (MCH) resource navigation.

Method: 方法上，论文主要采用或提出了如下思路：We present the design and implementation of a safety constrained large language model (LLM) system for public health information access, focusing on maternal and child health (MCH) resource navigation.

Result: 实验或分析结果显示：Results show consistent enforcement of safety constraints, reliable resource grounding, and stable system performance, with an average response time of 5.

Conclusion: 总体来看，论文结论是：Our findings provide practical guidance for deploying LLM based systems in healthcare and other domains where strict information boundaries and accountability are required.
