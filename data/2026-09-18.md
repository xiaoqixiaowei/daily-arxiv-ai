...
### [1] [Beyond Patch Removal: Persistent Adversarial Effects in Vision-Language-Action Policies](https://arxiv.org/abs/2609.19669)
...
TL;DR: 本文围绕《Beyond Patch Removal: Persistent Adversarial Effects in Vision-Language-Action Policies》展开，核心内容是：Adversarial patches to Vision-Language-Action (VLA) policies can cause both immediate action corruption and persistent state effects that remain after the patch is removed. Existing evaluations largely focus on continuous attacks and do not separate these two effects.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Adversarial patches to Vision-Language-Action (VLA) policies can cause both immediate action corruption and persistent state effects that remain after the patch is removed.

Method: 方法上，论文主要采用或提出了如下思路：We introduce a state-restoration protocol that removes the patch at matched action-chunk boundaries and measures subsequent recoverability under the same remaining step budget.

Result: 实验或分析结果显示：We also evaluate a recovery adapter trained on attack-induced states under controlled intervention latency.

Conclusion: 总体来看，论文结论是：These results show that adversarial effects can persist after patch removal and that timely intervention is critical for recovery.
...
TL;DR: 本文围绕《VABench: Measuring Embodied Spatial Intelligence through Visual Demonstrations, Active Perception, and Metric Control》展开，核心内容是：Spatial intelligence requires more than describing object locations. Under incomplete observation, models must identify and acquire missing evidence, interpret it in a common spatial frame, and act on it.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Spatial intelligence requires more than describing object locations.

Method: 方法上，论文主要采用或提出了如下思路：We introduce VA-Bench to evaluate the complete observe-reason-act-revise loop.

Result: 实验或分析结果显示：We introduce VA-Bench to evaluate the complete observe-reason-act-revise loop.

Conclusion: 总体来看，论文结论是：VA-Bench thus tests whether general-purpose MLLMs can turn visual demonstrations and actively acquired evidence into successful embodied action.
...
### [3] [Scene-Q: Confidence-Aware Coarse-to-Fine Querying of 3D Scenes with Selective VLM Reasoning](https://arxiv.org/abs/2609.20235)
...
TL;DR: 本文围绕《Scene-Q: Confidence-Aware Coarse-to-Fine Querying of 3D Scenes with Selective VLM Reasoning》展开，核心内容是：Indoor mobile robots require open-vocabulary scene understanding that grounds natural-language queries in a consistent 3D map. Many existing systems ultimately rely on cosine-similarity retrieval with contrastive image--text encoders, which is efficient but brittle when labels are near-synonymous or multiple similar instances appear.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Indoor mobile robots require open-vocabulary scene understanding that grounds natural-language queries in a consistent 3D map.

Method: 方法上，论文主要采用或提出了如下思路：Many existing systems ultimately rely on cosine-similarity retrieval with contrastive image--text encoders, which is efficient but brittle when labels are near-synonymous or multiple similar instances appear.

Result: 实验或分析结果显示：Scene-Q improves open-vocabulary 3D instance segmentation on ScanNet200 and natural-language 3D instance retrieval on real-world reconstructions, with the largest gains on spatial and relational queries while keeping a substantial fraction of queries on the fast path.

Conclusion: 总体来看，论文结论是：Scene-Q improves open-vocabulary 3D instance segmentation on ScanNet200 and natural-language 3D instance retrieval on real-world reconstructions, with the largest gains on spatial and relational queries while keeping a substantial fraction of queries on the fast path.
...
### [4] [VLN on the Fly: An Onboard Vision-Language Navigation Stack for Aerial Robots](https://arxiv.org/abs/2609.20191)
...
TL;DR: 本文围绕《VLN on the Fly: An Onboard Vision-Language Navigation Stack for Aerial Robots》展开，核心内容是：Running vision-language navigation fully onboard an aerial robot is hard, since grounding, planning, and control must share limited compute and a single-stage error is difficult to isolate in flight. End-to-end aerial policies fuse these stages into one network, giving up the observability and safety checks a modular stack keeps available.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Running vision-language navigation fully onboard an aerial robot is hard, since grounding, planning, and control must share limited compute and a single-stage error is difficult to isolate in flight.

Method: 方法上，论文主要采用或提出了如下思路：We propose VLN on the Fly, an onboard stack that keeps grounding, planning, and control as separate, inspectable stages.

Result: 实验或分析结果显示：We propose VLN on the Fly, an onboard stack that keeps grounding, planning, and control as separate, inspectable stages.

Conclusion: 总体来看，论文结论是：In 6 additional cluttered-environment trials, the stack tracks collision-free trajectories under onboard perception gating.
...
### [5] [Navi-Agent: Unlocalized Monocular Navigation Agent](https://arxiv.org/abs/2609.20388)
...
TL;DR: 本文围绕《Navi-Agent: Unlocalized Monocular Navigation Agent》展开，核心内容是：Vision-Language Navigation in Continuous Environments (VLN-CE) requires an embodied agent to execute long-horizon instructions in unknown environments. Existing zero-shot VLN-CE systems typically maintain spatial states through geometric localization or coordinate-based representations.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Recent geometry-constrained navigation removes depth and globally consistent coordinates, but maintaining persistent spatial awareness for place confirmation, progress verification, and recovery remains challenging.

Method: 方法上，论文主要采用或提出了如下思路：Existing zero-shot VLN-CE systems typically maintain spatial states through geometric localization or coordinate-based representations.

Result: 实验或分析结果显示：Experiments on zero-shot VLN-CE benchmark and real-world robot platforms show that Navi-Agent achieves state-of-the-art performance among geometry-constrained methods while remaining competitive with approaches relying on geometric localization.

Conclusion: 总体来看，论文结论是：Experiments on zero-shot VLN-CE benchmark and real-world robot platforms show that Navi-Agent achieves state-of-the-art performance among geometry-constrained methods while remaining competitive with approaches relying on geometric localization.
...
### [6] [GAVEL: Graph World Models for Verified and Efficient Long-Horizon LLM Task Planning](https://arxiv.org/abs/2609.19315)
...
TL;DR: 本文围绕《GAVEL: Graph World Models for Verified and Efficient Long-Horizon LLM Task Planning》展开，核心内容是：Large language models (LLMs) provide a flexible interface for long-horizon robot planning, but generated plans often fail to respect embodiment constraints, recover from planning errors, or reason effectively under partial observability. We present GAVEL, a framework for verifying and repairing long-horizon LLM planning built around an explicit graph world model.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：For multi-task instructions, GAVEL reasons over distributions of possible object locations to reorder remaining subtasks and minimize expected search cost.

Method: 方法上，论文主要采用或提出了如下思路：We present GAVEL, a framework for verifying and repairing long-horizon LLM planning built around an explicit graph world model.

Result: 实验或分析结果显示：We evaluate GAVEL on BEHAVIOR-1K across 100 single long-horizon tasks and 500 multi-task instructions.

Conclusion: 总体来看，论文结论是：These improvements show that an explicit graph world model harness can substantially improve the reliability and efficiency of long-horizon embodied planning across compact and frontier hosted LLM capabilities.
...
### [7] [From Rollout to Reset: A Graph-Based Harness for Autonomous Long-Horizon Manipulation Evaluation](https://arxiv.org/abs/2609.19413)
...
TL;DR: 本文围绕《From Rollout to Reset: A Graph-Based Harness for Autonomous Long-Horizon Manipulation Evaluation》展开，核心内容是：Robot manipulation policies are improving quickly, and real-robot evaluation remains the standard evidence for that progress. It still relies on a human to reset the scene between rollouts, which consumes operator time and leaves the initial state distribution unspecified, so results reproduce poorly.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Robot manipulation policies are improving quickly, and real-robot evaluation remains the standard evidence for that progress.

Method: 方法上，论文主要采用或提出了如下思路：A recent system, AutoEval, automates both reset and scoring, but only for single-step tasks, because a long-horizon rollout can terminate in combinatorially many configurations that no single learned reset policy covers.

Result: 实验或分析结果显示：It still relies on a human to reset the scene between rollouts, which consumes operator time and leaves the initial state distribution unspecified, so results reproduce poorly.

Conclusion: 总体来看，论文结论是：3% for a per-task reset policy, and we ablate the scene representation and the graph update rate.
...
### [8] [SIMLIFE: Pattern Understanding for Long-Horizon Human-Agent Partnership](https://arxiv.org/abs/2609.19610)
...
TL;DR: 本文围绕《SIMLIFE: Pattern Understanding for Long-Horizon Human-Agent Partnership》展开，核心内容是：Understanding humans over long horizons requires agents to infer not only what people need in the moment, but also how routines form, why they repeat, and when they change. We introduce SimLife, a scalable platform for simulating long-term household life with rich visual observations, ground-truth action logs, and synthetic dialogues with audio.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：These findings suggest that long-context pattern understanding remains a major bottleneck for future embodied agents, while SimLife opens a broader space for studying memory, personalization, adaptation, and long-horizon planning in everyday human-AI interaction.

Method: 方法上，论文主要采用或提出了如下思路：We introduce SimLife, a scalable platform for simulating long-term household life with rich visual observations, ground-truth action logs, and synthetic dialogues with audio.

Result: 实验或分析结果显示：Built on SimLife, SimLife-BP evaluates long-context pattern understanding: the ability to infer latent behavioral rules from weeks or months of everyday observations.

Conclusion: 总体来看，论文结论是：These findings suggest that long-context pattern understanding remains a major bottleneck for future embodied agents, while SimLife opens a broader space for studying memory, personalization, adaptation, and long-horizon planning in everyday human-AI interaction.
...
### [9] [A Dual-Process Perspective on Nudge Susceptibility in LLM-Based GUI Agents](https://arxiv.org/abs/2609.19843)
...
TL;DR: 本文围绕《A Dual-Process Perspective on Nudge Susceptibility in LLM-Based GUI Agents》展开，核心内容是：LLM-based GUI agents increasingly act on behalf of users in digital environments that were designed with human users in mind. These graphical user interfaces were designed to support, but also deliberately steer, the behaviour and decisions of users.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：LLM-based GUI agents increasingly act on behalf of users in digital environments that were designed with human users in mind.

Method: 方法上，论文主要采用或提出了如下思路：Exploratory analysis further showed this redirection to be systematically structured by model scale.

Result: 实验或分析结果显示：In a randomized online shopping experiment with 3,600 agents and a total of 21,600 simulations across six frontier models from three providers, we found that agents were vulnerable to both nudge types.

Conclusion: 总体来看，论文结论是：Beyond establishing nudge susceptibility as a behavioural property of agentic AI, the study positions interface design as a governance concern for organizations that delegate decisions to autonomous agents.
...
### [10] [LLM-Guided Transformation of Non-Critical Driving Scenes into Safety-Critical Scenarios Using Augmented Reality](https://arxiv.org/abs/2609.20318)
...
TL;DR: 本文围绕《LLM-Guided Transformation of Non-Critical Driving Scenes into Safety-Critical Scenarios Using Augmented Reality》展开，核心内容是：Testing Autonomous Driving Systems (ADS) requires realistic safety-critical scenarios, but collecting such data from real-world driving is costly and unsafe. This paper presents an automated pipeline that transforms safe driving scenes into safety-critical scenarios by combining computer vision, Large Language Models (LLMs), and Augmented Reality (AR).


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Testing Autonomous Driving Systems (ADS) requires realistic safety-critical scenarios, but collecting such data from real-world driving is costly and unsafe.

Method: 方法上，论文主要采用或提出了如下思路：Testing Autonomous Driving Systems (ADS) requires realistic safety-critical scenarios, but collecting such data from real-world driving is costly and unsafe.

Result: 实验或分析结果显示：The proposed pipeline was evaluated on the nuScenes dataset, achieving 97.

Conclusion: 总体来看，论文结论是：The results demonstrate an effective and flexible approach for automated generation of safety-critical scenarios to support the testing and validation of autonomous driving systems.
...
### [11] [HIL-UMI: Bringing Human-in-the-Loop Post-Training of Vision-Language-Action Models to Universal Manipulation Interface](https://arxiv.org/abs/2609.20659)
...
TL;DR: 本文围绕《HIL-UMI: Bringing Human-in-the-Loop Post-Training of Vision-Language-Action Models to Universal Manipulation Interface》展开，核心内容是：Large-scale vision-language-action (VLA) models provide powerful priors for robot manipulation, yet adapting them to a specific deployment remains challenging. Supervised fine-tuning (SFT) on task-specific demonstrations provides a step toward deployment, but faces two persistent limitations: static data provide limited coverage of out-of-distribution states, and standard imitation objectives do not distinguish progressing behavior from less useful data.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Large-scale vision-language-action (VLA) models provide powerful priors for robot manipulation, yet adapting them to a specific deployment remains challenging.

Method: 方法上，论文主要采用或提出了如下思路：We introduce HIL-UMI, a policy-guided Universal Manipulation Interface (UMI) framework for robot-free human-in-the-loop VLA post-training.

Result: 实验或分析结果显示：Experiments on four real-world tasks spanning long-horizon and precise manipulation show that HIL-UMI achieves consistent improvement over SFT and benefits from both targeted collection and advantage refinement.

Conclusion: 总体来看，论文结论是：Moreover, HIL-UMI outperforms HG-DAgger on Clean Up Table with lower per-frame collection time, suggesting a scalable path for VLA post-training across operators and locations.
...
### [12] [Workspace Models: Lightweight Robotic Memory via Saliency-Driven Supervision](https://arxiv.org/abs/2609.20820)
...
TL;DR: 本文围绕《Workspace Models: Lightweight Robotic Memory via Saliency-Driven Supervision》展开，核心内容是：Complex robotic manipulation tasks frequently require a long-term memory of past events and actions. As conditioning on full histories renders policies prone to spurious correlations and degrades performance, many approaches to policy memory involve compressing historical information through expensive VLM queries in-the-loop to process only task-salient information.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Complex robotic manipulation tasks frequently require a long-term memory of past events and actions.

Method: 方法上，论文主要采用或提出了如下思路：In this paper, we propose an alternative approach in which computationally intensive VLM queries are made during train-time to learn a lightweight latent memory that can be efficiently queried at deployment time.

Result: 实验或分析结果显示：In both simulation and hardware, we show that the workspace token can be used as a drop-in replacement for observations during deployment, enabling policies to solve memory-intensive tasks without the need for VLM reasoning in-the-loop.

Conclusion: 总体来看，论文结论是：Interestingly, we found that workspace tokens are not only more lightweight but also lead to better policy performance.
...
### [13] [From Wizard-of-Oz Human-Robot Dialogue Collection to a Taxonomy of Robot Response Decisions: A Retrospective Analysis of Assistive Pilot Interactions](https://arxiv.org/abs/2609.19447)
...
TL;DR: 本文围绕《From Wizard-of-Oz Human-Robot Dialogue Collection to a Taxonomy of Robot Response Decisions: A Retrospective Analysis of Assistive Pilot Interactions》展开，核心内容是：Robots that follow natural-language instructions in everyday indoor environments must act on incomplete human utterances. Instructions often omit essential information, such as the identity of an out-of-view object, an intended destination, or the user's goal.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Remaining boundary cases in decision-point identification and REPORT_DONE motivate a constrained protocol for more consistent dialogue collection.

Method: 方法上，论文主要采用或提出了如下思路：Instructions often omit essential information, such as the identity of an out-of-view object, an intended destination, or the user's goal.

Result: 实验或分析结果显示：Existing datasets contain little real-world situated dialogue and provide few practice-grounded criteria for deciding when a robot should act, confirm, clarify, or refuse.

Conclusion: 总体来看，论文结论是：Remaining boundary cases in decision-point identification and REPORT_DONE motivate a constrained protocol for more consistent dialogue collection.
...
### [14] [Pose-aware Legged Robot Semantic Exploration with Omnidirectional Perception in Confined Unknown Environments](https://arxiv.org/abs/2609.19460)
...
TL;DR: 本文围绕《Pose-aware Legged Robot Semantic Exploration with Omnidirectional Perception in Confined Unknown Environments》展开，核心内容是：Semantic exploration in confined environments requires both environment mapping and detailed observation of target objects. For ground robots, limited sensor vertical fields of view and restricted standoff distances can leave upper object surfaces unobserved from planar viewpoints.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Semantic exploration in confined environments requires both environment mapping and detailed observation of target objects.

Method: 方法上，论文主要采用或提出了如下思路：To address this trade-off, we present POSE, a pose-aware semantic exploration system that exploits a legged robot's intrinsic body pitch and roll with omnidirectional camera-LiDAR perception.

Result: 实验或分析结果显示：Body tilting can improve coverage, but additional observations and posture transitions increase mission time.

Conclusion: 总体来看，论文结论是：We plan to release the code for community benefit in the future.
...
### [15] [FASA: Feedback-Aware Sampling Adaptation for Efficient Diffusion-Based VLA Models](https://arxiv.org/abs/2609.19475)
...
TL;DR: 本文围绕《FASA: Feedback-Aware Sampling Adaptation for Efficient Diffusion-Based VLA Models》展开，核心内容是：Diffusion-based Vision-Language-Action (VLA) models achieve strong performance in embodied tasks, but their iterative sampling imposes heavy computational and memory-access cost, blocking real-time deployment on edge platforms. Existing acceleration methods either require expensive training (e.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Diffusion-based Vision-Language-Action (VLA) models achieve strong performance in embodied tasks, but their iterative sampling imposes heavy computational and memory-access cost, blocking real-time deployment on edge platforms.

Method: 方法上，论文主要采用或提出了如下思路：This paper presents FASA (Feedback-Aware Sampling Adaptation), a training-free runtime framework that treats real-time multimodal feedback as a control signal for the denoising pipeline: an interaction-driven range adaptor modulates the global sampling-step budget based on visual and gripper-force feedback, and a proprioception-aware step adaptor pinpoints the optimized step within the adapted range.

Result: 实验或分析结果显示：Diffusion-based Vision-Language-Action (VLA) models achieve strong performance in embodied tasks, but their iterative sampling imposes heavy computational and memory-access cost, blocking real-time deployment on edge platforms.

Conclusion: 总体来看，论文结论是：45$\times$ while maintaining competitive success rates, providing a novel dynamic runtime architecture paradigm for deploying heavy generative embodied AI workloads onto resource-constrained computing platforms.
...
### [16] [Navigate or Relocate? Planning Among Movable Obstacles in Unknown Environments](https://arxiv.org/abs/2609.19541)
...
TL;DR: 本文围绕《Navigate or Relocate? Planning Among Movable Obstacles in Unknown Environments》展开，核心内容是：Conventional robot planning methods seek collision-free paths to a goal but fail when all paths are blocked. In these cases, the robot must determine which objects to relocate, in what order, and where to place them to clear a path---a problem known as Navigation Among Movable Obstacles (NAMO).


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：In these cases, the robot must determine which objects to relocate, in what order, and where to place them to clear a path---a problem known as Navigation Among Movable Obstacles (NAMO).

Method: 方法上，论文主要采用或提出了如下思路：We propose an online framework that addresses this ambiguity by selecting between navigation and relocation using shortest paths that treat discovered movable objects as obstacles or as removable.

Result: 实验或分析结果显示：Numerical experiments demonstrate scalability to cluttered environments requiring interdependent relocations and improved plan quality over existing baselines.

Conclusion: 总体来看，论文结论是：Numerical experiments demonstrate scalability to cluttered environments requiring interdependent relocations and improved plan quality over existing baselines.
...
### [17] [Recovering Aggressively Pruned Vision-Language-Action Models with Offline Hidden-State Distillation](https://arxiv.org/abs/2609.19579)
...
TL;DR: 本文围绕《Recovering Aggressively Pruned Vision-Language-Action Models with Offline Hidden-State Distillation》展开，核心内容是：Vision-language-action (VLA) models let robots follow language instructions, but their language backbones of several billion parameters are the main obstacle to running them on robot hardware. Structured pruning reduces that backbone, and removing 63% of it from OpenVLA-OFT drops LIBERO-Long success from 93.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：At 81% on CogACT, a tripled recovery budget narrows the distilled student's gap to the teacher to 3.

Method: 方法上，论文主要采用或提出了如下思路：Structured pruning reduces that backbone, and removing 63% of it from OpenVLA-OFT drops LIBERO-Long success from 93.

Result: 实验或分析结果显示：At matched compression, width pruning yields higher success and depth pruning lower latency.

Conclusion: 总体来看，论文结论是：23x faster on-board than the teacher, and uses 62% less memory.
...
### [18] [WorldContact: A Contact-Centric World Model for Scalable Robot Learning](https://arxiv.org/abs/2609.19600)
...
TL;DR: 本文围绕《WorldContact: A Contact-Centric World Model for Scalable Robot Learning》展开，核心内容是：Adapting robots to new objects and tasks requires interaction experience that can be costly to obtain. We present WorldContact, a contact-centric world model for deformable-object manipulation, constructed from a limited set of high-quality trajectories to generate additional training data efficiently.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Adapting robots to new objects and tasks requires interaction experience that can be costly to obtain.

Method: 方法上，论文主要采用或提出了如下思路：We present WorldContact, a contact-centric world model for deformable-object manipulation, constructed from a limited set of high-quality trajectories to generate additional training data efficiently.

Result: 实验或分析结果显示：We evaluate WorldContact across 16 shopping-bag manipulation tasks.

Conclusion: 总体来看，论文结论是：These results support efficient data generation with WorldContact for robot policy adaptation.
...
### [19] [ReShoot: Generative Visual Domain Randomization of Recorded Robot Demonstrations for Visuomotor Policy Learning](https://arxiv.org/abs/2609.19661)
...
TL;DR: 本文围绕《ReShoot: Generative Visual Domain Randomization of Recorded Robot Demonstrations for Visuomotor Policy Learning》展开，核心内容是：Imitation-learned robot policies are frequently overfit to the visual conditions present in their training demonstrations. Consequently, variations in object color or background appearance often induce substantial performance degradation.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Imitation-learned robot policies are frequently overfit to the visual conditions present in their training demonstrations.

Method: 方法上，论文主要采用或提出了如下思路：Imitation-learned robot policies are frequently overfit to the visual conditions present in their training demonstrations.

Result: 实验或分析结果显示：Moreover, the mixed training set improves robustness to scene perturbations on LIBERO-Plus (85.

Conclusion: 总体来看，论文结论是：5%, respectively, while maintaining performance under the original recorded appearance.
...
### [20] [Towards High-DoF Dexterous Manipulation through VLA Post-Training](https://arxiv.org/abs/2609.19666)
...
TL;DR: 本文围绕《Towards High-DoF Dexterous Manipulation through VLA Post-Training》展开，核心内容是：Imitation-learned vision--language--action (VLA) foundation models acquire broad manipulation capabilities by scaling robot data across tasks and embodiments, but reliable deployment on a specific downstream task and hardware platform still requires post-training. Dexterous hands make this adaptation particularly difficult: their broad behavioural repertoire and high degree of freedom create a large and structured action space.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Imitation-learned vision--language--action (VLA) foundation models acquire broad manipulation capabilities by scaling robot data across tasks and embodiments, but reliable deployment on a specific downstream task and hardware platform still requires post-training.

Method: 方法上，论文主要采用或提出了如下思路：We present a unified four-step post-training pipeline comprising a learned temporal hand-action codec, supervised fine-tuning, DAgger, and real-world residual reinforcement learning.

Result: 实验或分析结果显示：We evaluate the pipeline on five diverse real-world tasks spanning bimanual transfer, in-hand reorientation, and tool use.

Conclusion: 总体来看，论文结论是：These results provide a practical path for adapting VLA foundation models to reliable real-world dexterous manipulation.
...
### [21] [TADreamer: Zero-Shot Language-Guided 3D Navigation for Terrestrial-Aerial Bimodal Robots via Video Imagination](https://arxiv.org/abs/2609.19824)
...
TL;DR: 本文围绕《TADreamer: Zero-Shot Language-Guided 3D Navigation for Terrestrial-Aerial Bimodal Robots via Video Imagination》展开，核心内容是：Language-guided navigation for terrestrial-aerial bimodal robots requires selecting routes and locomotion modes that match scene context and task intent. Generated videos can represent such motion sequences, but recovering metrically consistent navigation references from them is challenging because of scale ambiguity and axis-dependent geometric distortions.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Language-guided navigation for terrestrial-aerial bimodal robots requires selecting routes and locomotion modes that match scene context and task intent.

Method: 方法上，论文主要采用或提出了如下思路：Generated videos can represent such motion sequences, but recovering metrically consistent navigation references from them is challenging because of scale ambiguity and axis-dependent geometric distortions.

Result: 实验或分析结果显示：Real-world experiments demonstrate navigation across seven indoor and outdoor scenarios.

Conclusion: 总体来看，论文结论是：3% compared with NavDreamer.
...
TL;DR: 本文围绕《Co-VLA: Consensus-based Federated Training for Vision-Language-Action Models》展开，核心内容是：Vision-language-action models (VLAs) have emerged as a promising paradigm for general-purpose robot learning, with performance improving as models and datasets scale. Scaling robot data collection, however, remains challenging because data are naturally distributed across robots, tasks, and locations, making centralization costly or impractical.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Scaling robot data collection, however, remains challenging because data are naturally distributed across robots, tasks, and locations, making centralization costly or impractical.

Method: 方法上，论文主要采用或提出了如下思路：We present Co-VLA, which applies consensus optimization using the Alternating Direction Method of Multipliers~(ADMM) to federated VLA training.

Result: 实验或分析结果显示：We show that the same algorithm supports both full-model training and parameter-efficient fine-tuning with both fixed-rank and rank-adaptive adapters.

Conclusion: 总体来看，论文结论是：Our experiments demonstrate that Co-VLA achieves performance comparable to centralized training in both full-model training and parameter-efficient fine-tuning settings.
...
### [23] [Compliance for Free: Learning Identifiable Impedance via Bilateral Teleoperation](https://arxiv.org/abs/2609.19976)
...
TL;DR: 本文围绕《Compliance for Free: Learning Identifiable Impedance via Bilateral Teleoperation》展开，核心内容是：Vision-language-action models tell a robot where to move, but not how hard to push. Contact-rich tasks depend on that second quantity, compliance, yet no widely used demonstration interface records it.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Four-channel bilateral teleoperation removes the ambiguity directly by using the leader arm as a separate measurement of the intended equilibrium, making per-axis stiffness identifiable by regression using only the joint-torque sensing already on the manipulator.

Method: 方法上，论文主要采用或提出了如下思路：Contact-rich tasks depend on that second quantity, compliance, yet no widely used demonstration interface records it.

Result: 实验或分析结果显示：This yields per-timestep, direction-dependent compliance labels at zero annotation cost, which we use to fine-tune a VLA to emit stiffness alongside pose.

Conclusion: 总体来看，论文结论是：023
...
### [24] [Imagine-TAMP: Imagination-Guided Task and Motion Planning in Partial Observability](https://arxiv.org/abs/2609.20396)
...
TL;DR: 本文围绕《Imagine-TAMP: Imagination-Guided Task and Motion Planning in Partial Observability》展开，核心内容是：Robots operating in cluttered environments must often manipulate objects whose locations are only partially observable. A central challenge is deciding whether to acquire another observation or to first manipulate objects that may occlude the target.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：A central challenge is deciding whether to acquire another observation or to first manipulate objects that may occlude the target.

Method: 方法上，论文主要采用或提出了如下思路：We introduce Imagine-TAMP, an interleaved planning and execution framework that uses semantic and geometric imagination to compare alternative task-level strategies under partial observability before committing to expensive motion planning.

Result: 实验或分析结果显示：Experiments show that imagination-guided evaluation improves observation-versus-manipulation decisions: in viewpoint-constrained shelf scenes, non-unit geometric evaluation increases success from 46.

Conclusion: 总体来看，论文结论是：On a real robot, the complete system reduces planning time by 32% relative to a geometry-only ablation.
...
### [25] [Spatial-Semantic Uncertainty in VLM-Based Target Search: Balancing Exploration and Identification](https://arxiv.org/abs/2609.20443)
...
TL;DR: 本文围绕《Spatial-Semantic Uncertainty in VLM-Based Target Search: Balancing Exploration and Identification》展开，核心内容是：Robots searching for a target from a natural-language description must determine not only where to search, but also which observed candidate is the desired target. These decisions reflect two distinct sources of uncertainty - spatial uncertainty over candidate locations and semantic uncertainty over target identity - that are often conflated in VLM-based search systems.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Robots searching for a target from a natural-language description must determine not only where to search, but also which observed candidate is the desired target.

Method: 方法上，论文主要采用或提出了如下思路：These decisions reflect two distinct sources of uncertainty - spatial uncertainty over candidate locations and semantic uncertainty over target identity - that are often conflated in VLM-based search systems.

Result: 实验或分析结果显示：We evaluate six VLM uncertainty-elicitation interfaces on 500 synthetic targets and show that similar recognition accuracy can conceal substantial differences in calibration and false confidence.

Conclusion: 总体来看，论文结论是：These results highlight the distinct roles of uncertainty representation and uncertainty-driven planning in embodied VLM systems.
...
### [26] [V2-STRep: VLM-Grounded Structured Task Representations for Reusable Robot Skills Acquired from Generated Videos](https://arxiv.org/abs/2609.20582)
...
TL;DR: 本文围绕《V2-STRep: VLM-Grounded Structured Task Representations for Reusable Robot Skills Acquired from Generated Videos》展开，核心内容是：Human manipulation videos provide rich motion and interaction cues for acquiring robot skills without robot demonstrations. Video generation models synthesize such demonstrations from an initial scene image and task instruction, avoiding the need to record demonstrations for each task.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：It preserves task requirements while using remaining rotational freedom to accommodate joint limits.

Method: 方法上，论文主要采用或提出了如下思路：We present V2-STRep, a zero-shot framework that converts generated video motion into reusable robot skills through VLM-grounded structured task representations.

Result: 实验或分析结果显示：Experiments on six real-world manipulation tasks demonstrate improved execution success over baselines, reliable cross-scene transfer of successfully acquired skills, and adaptation to changed deployment instructions.

Conclusion: 总体来看，论文结论是：Experiments on six real-world manipulation tasks demonstrate improved execution success over baselines, reliable cross-scene transfer of successfully acquired skills, and adaptation to changed deployment instructions.
...
### [27] [TraceFlow: Guiding Frozen Flow-Matching Robot Policies with Success and Failure Traces](https://arxiv.org/abs/2609.20646)
...
TL;DR: 本文围绕《TraceFlow: Guiding Frozen Flow-Matching Robot Policies with Success and Failure Traces》展开，核心内容是：A vision-language-action (VLA) policy with a flow-matching action expert generates each action chunk (a short command sequence) by integrating a learned velocity field; once its weights are fixed, the success or failure of an earlier rollout cannot change the chunk generated now. Concurrent test-time methods give a frozen policy such an input from retrieved successes, a learned critic, a verifier, or a dynamics model, but none uses the robot's own failed rollouts as negative evidence with nothing but a terminal outcome bit.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：A vision-language-action (VLA) policy with a flow-matching action expert generates each action chunk (a short command sequence) by integrating a learned velocity field; once its weights are fixed, the success or failure of an earlier rollout cannot change the chunk generated now.

Method: 方法上，论文主要采用或提出了如下思路：We introduce TraceFlow, a progress-aligned guidance field that turns the action densities of retrieved successful and failed rollouts into a bounded correction to a frozen flow-matching action expert, using one terminal outcome bit per rollout and no other label.

Result: 实验或分析结果显示：We introduce TraceFlow, a progress-aligned guidance field that turns the action densities of retrieved successful and failed rollouts into a bounded correction to a frozen flow-matching action expert, using one terminal outcome bit per rollout and no other label.

Conclusion: 总体来看，论文结论是：Stacking gains are finite, every branch peaking before round ten, and the bank's success-to-failure ratio predicts no retrieval allocation.
...
### [28] [SkipVLA: Skipping VLA Steps with Classical Planning for Fast Robot Manipulation](https://arxiv.org/abs/2609.20648)
...
TL;DR: 本文围绕《SkipVLA: Skipping VLA Steps with Classical Planning for Fast Robot Manipulation》展开，核心内容是：Vision-Language-Action (VLA) models are a class of generalist robot policies that map camera images and language instructions directly to robot actions. While promising, these models remain slow at test time, particularly for long-horizon tasks that require many queries to the policy.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：While promising, these models remain slow at test time, particularly for long-horizon tasks that require many queries to the policy.

Method: 方法上，论文主要采用或提出了如下思路：In this work, we present SkipVLA, a hybrid policy that combines a pretrained VLA with a classical motion planner, using the planner for free-space motion and querying the VLA only for contact-rich skills such as grasping and placing.

Result: 实验或分析结果显示：We evaluate SkipVLA with three VLAs on 13 LIBERO tasks in simulation and three pick-and-place tasks on a physical 6-DoF YAM arm, demonstrating up to 2.

Conclusion: 总体来看，论文结论是：5x faster task completion and significantly lower energy consumption while achieving the same task success rate.
...
TL;DR: 本文围绕《StageGuard: Learning Stage Transitions for Long-Horizon Robot Tasks via Agentic Distillation》展开，核心内容是：Hierarchical planning frameworks combine skills from multiple robot control policies for long-horizon task execution, where determining when to terminate the current skill and advance to the next subtask is essential. Existing approaches often rely on pre-designed completion signal checkers that are hard to obtain in real-world execution.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Hierarchical planning frameworks combine skills from multiple robot control policies for long-horizon task execution, where determining when to terminate the current skill and advance to the next subtask is essential.

Method: 方法上，论文主要采用或提出了如下思路：Hierarchical planning frameworks combine skills from multiple robot control policies for long-horizon task execution, where determining when to terminate the current skill and advance to the next subtask is essential.

Result: 实验或分析结果显示：We evaluate stage-transition prediction on trajectories from two benchmarks and assess closed-loop task success through integration into hierarchical robot control on BEHAVIOR-1K, with further validation on real robots.

Conclusion: 总体来看，论文结论是：Results show substantial improvements in stage-transition prediction while supporting efficient online monitoring.
...
### [30] [EconSkills: Studying Skill Transfer and Retrieval for Web Agents on Live Economic Data](https://arxiv.org/abs/2609.19523)
...
TL;DR: 本文围绕《EconSkills: Studying Skill Transfer and Retrieval for Web Agents on Live Economic Data》展开，核心内容是：Web agents often revisit the same sites, yet most evaluations discard the procedures learned in earlier successful interactions. We introduce EconSkills, a skill library and evaluation framework that distills verified EconWebArena trajectories into parameterized standard operating procedures for retrieving live economic data.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Browser trajectories further identify when procedural guidance shortens portal-specific navigation and when semantic verification remains necessary.

Method: 方法上，论文主要采用或提出了如下思路：We introduce EconSkills, a skill library and evaluation framework that distills verified EconWebArena trajectories into parameterized standard operating procedures for retrieving live economic data.

Result: 实验或分析结果显示：In controlled transfer, matched skills improve success over no-skill prompting and require fewer steps on paired successes, while abstraction is substantially more effective than replaying raw trajectories.

Conclusion: 总体来看，论文结论是：These results establish that reusable economic web procedures can transfer across task instances and provide a concrete design target for coverage-aware selection and context delivery.
...
### [31] [MAGS: Multi-agent Auto-formalization Guarantees Safety for Agentic Outputs](https://arxiv.org/abs/2609.19391)
...
TL;DR: 本文围绕《MAGS: Multi-agent Auto-formalization Guarantees Safety for Agentic Outputs》展开，核心内容是：LLM coding agents now generate complex programs at a scale that makes thorough human review increasingly difficult, raising the risk of safety and security failures. Common approaches, including fuzz testing, static analysis, and LLM-as-a-Verifier, can detect many failures but struggle to cover all possible edge cases.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：LLM coding agents now generate complex programs at a scale that makes thorough human review increasingly difficult, raising the risk of safety and security failures.

Method: 方法上，论文主要采用或提出了如下思路：We introduce a unified multi-agent framework, MAGS, that generates executable programs with formal safety guarantees, using Dafny as a verification-aware intermediate representation where safety properties can be mechanically checked.

Result: 实验或分析结果显示：We evaluate MAGS on 100 CUDA kernels, 100 terminal scripts, and 20 robotic-arm tasks.

Conclusion: 总体来看，论文结论是：Independent safety and functional evaluations further show strong performance across all three domains, while revealing failures when the auto-formalized semantics do not fully capture the target behavior.
...
### [32] [Kinematics-Grounded Agentic AI for Robotic Additive Manufacturing Process Planning](https://arxiv.org/abs/2609.19347)
...
TL;DR: 本文围绕《Kinematics-Grounded Agentic AI for Robotic Additive Manufacturing Process Planning》展开，核心内容是：Robotic additive manufacturing (AM) extends material-extrusion printing beyond gantry kinematics but makes process planning robot-dependent. A slicer-generated plan that appears favorable in part coordinates can become infeasible or robotically unfavorable on a manipulator because slicer-process decisions and part orientation determine the generated path, while part orientation and workspace placement affect its kinematic realization.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：This paper presents agentic robotic additive manufacturing (A-RAM), an agent-specialist-tool framework that converts user intent and a part file into traceable, execution-ready plans.

Method: 方法上，论文主要采用或提出了如下思路：Existing AM tools, large language model (LLM)-based decision-support methods, and digital-shadow systems do not provide integrated pre-execution evaluation of these coupled decisions.

Result: 实验或分析结果显示：The framework is evaluated on a six-axis robotic-arm AM cell through three case studies covering expert-specified planning, goal-only planning, objective-dependent infill screening, and geometry-dependent orientation-placement selection.

Conclusion: 总体来看，论文结论是：7% shorter than the corresponding least favorable screened patterns.
...
### [33] [Uni-LaDiR: Latent Diffusion Unifies Multimodal Reasoning](https://arxiv.org/abs/2609.19878)
...
TL;DR: 本文围绕《Uni-LaDiR: Latent Diffusion Unifies Multimodal Reasoning》展开，核心内容是：Multimodal reasoning requires models to draw on information from multiple modalities throughout the reasoning process. Yet existing methods often concatenate modality-specific thought tokens in a single sequence, leaving the model to bridge representational differences as it reasons across modalities.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Multimodal reasoning requires models to draw on information from multiple modalities throughout the reasoning process.

Method: 方法上，论文主要采用或提出了如下思路：Yet existing methods often concatenate modality-specific thought tokens in a single sequence, leaving the model to bridge representational differences as it reasons across modalities.

Result: 实验或分析结果显示：Across eleven vision-language model (VLM) benchmarks and two vision-language-action (VLA) suites, Uni-LaDiR achieves relative gains over the strongest evaluated baselines of 7.

Conclusion: 总体来看，论文结论是：1% on robot manipulation tasks.
...
### [34] [GeoAAC: Geometry-Based Adaptive Action Chunking from Denoising Trajectories in VLA Policies](https://arxiv.org/abs/2609.20776)
...
TL;DR: 本文围绕《GeoAAC: Geometry-Based Adaptive Action Chunking from Denoising Trajectories in VLA Policies》展开，核心内容是：Action chunking is widely used for action generation and execution in Vision-Language-Action (VLA) policies, yet existing approaches commonly use a fixed action horizon. During a rollout, different task stages may require different levels of action continuity, control precision, and closed-loop feedback, making a fixed horizon unable to accommodate changing control requirements.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：We show that the geometry of Flow Matching denoising trajectories provides process-level information for characterizing prediction reliability, with geometric variation across action prefixes remaining positively correlated with predictive uncertainty.

Method: 方法上，论文主要采用或提出了如下思路：We propose \textbf{GeoAAC}, a geometry-based adaptive action chunking method for flow-based VLA policies that adjusts the action horizon according to the reliability of the current action prediction.

Result: 实验或分析结果显示：We show that the geometry of Flow Matching denoising trajectories provides process-level information for characterizing prediction reliability, with geometric variation across action prefixes remaining positively correlated with predictive uncertainty.
