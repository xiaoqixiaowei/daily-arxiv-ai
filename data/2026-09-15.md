...
### [1] [PuzzleMate: Benchmarking MLLMs for Egocentric Puzzle Assistance](https://arxiv.org/abs/2609.14473)
...
TL;DR: 本文围绕《PuzzleMate: Benchmarking MLLMs for Egocentric Puzzle Assistance》展开，核心内容是：Personal AI assistants hold the potential to evolve from digital interfaces into embodied companions capable of guiding users through complex physical activities. For these assistants to become integral to daily life, they must do more than identify objects; they must provide precise, step-by-step instructions that align with a user's real-time progress.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：While Multimodal Large Language Models (MLLMs) show promise in general visual understanding, their ability to deliver grounded, sequential guidance for fine-grained manipulation tasks remains largely unverified.

Method: 方法上，论文主要采用或提出了如下思路：We investigate this capability through PuzzleMate, a novel framework focused on jigsaw puzzle solving captured through an egocentric viewpoint.

Result: 实验或分析结果显示：While Multimodal Large Language Models (MLLMs) show promise in general visual understanding, their ability to deliver grounded, sequential guidance for fine-grained manipulation tasks remains largely unverified.

Conclusion: 总体来看，论文结论是：5-Pro; while these MLLMs are highly capable, they struggle to navigate the intricate reasoning and sequential logic essential for jigsaw puzzle assistance.
...
### [2] [Learning from Reliable Negatives: Confidence-Anchored Test-Time Adaptation for GUI Grounding](https://arxiv.org/abs/2609.15307)
...
TL;DR: 本文围绕《Learning from Reliable Negatives: Confidence-Anchored Test-Time Adaptation for GUI Grounding》展开，核心内容是：Graphical User Interface (GUI) grounding is essential for autonomous agents to map natural language instructions to precise screen coordinates. However, existing supervised fine-tuning and reinforcement learning methods are constrained by the high cost of annotation, creating a scalability bottleneck.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Graphical User Interface (GUI) grounding is essential for autonomous agents to map natural language instructions to precise screen coordinates.

Method: 方法上，论文主要采用或提出了如下思路：In this paper, we introduce a label-free test-time training paradigm driven by two key insights: (1) confidence patterns in coordinate tokens are a better indicator than full-sequence confidence, and (2) in sparse GUI coordinate spaces, negative samples offer more reliable learning signals than potentially noisy positive ones.

Result: 实验或分析结果显示：Experimental results demonstrate that CANL-7B achieves 92.

Conclusion: 总体来看，论文结论是：Our findings establish coordinate-token confidence as a powerful alternative to manual annotations for scalable GUI agent development.
...
### [3] [AnchorGUI: Asymmetric Memory for Dual-Scale Learning in GUI Navigation](https://arxiv.org/abs/2609.15457)
...
TL;DR: 本文围绕《AnchorGUI: Asymmetric Memory for Dual-Scale Learning in GUI Navigation》展开，核心内容是：Vision-Language Models (VLMs) enable autonomous GUI navigation, but agents still struggle to process and learn from dense, continuous visual histories. This bottleneck hinders both immediate error correction within a single episode (intra-trial) and experience distillation across multiple attempts (cross-trial).


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：We trace these challenges to an empirical informational asymmetry in GUI navigation: while expected transitions can often be compressed into lightweight textual summaries, unexpected outcomes benefit from preserved screenshots as causal evidence for accurate diagnosis.

Method: 方法上，论文主要采用或提出了如下思路：Building on this insight, we propose AnchorGUI, a unified framework driven by the Cognitive State Anchor (CSA).

Result: 实验或分析结果显示：Experiments across four benchmarks validate the effectiveness of our approach.

Conclusion: 总体来看，论文结论是：9% gain), significantly outperforming standard reflection methods while maintaining sub-linear context scaling.
...
TL;DR: 本文围绕《Token Efficient Task Execution via Application Behavior Modeling for Web Agents》展开，核心内容是：The strong performance of AI Agents across an impressive variety of tasks is driving an unprecedented investment in agentic infrastructures, however the cost of processing tokens is fast increasing. Web agents automate the execution of web-application tasks described in natural language, by analyzing the web-application's user interface (UI) and interacting with it.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：The strong performance of AI Agents across an impressive variety of tasks is driving an unprecedented investment in agentic infrastructures, however the cost of processing tokens is fast increasing.

Method: 方法上，论文主要采用或提出了如下思路：This work introduces OdoBot, a novel web-agent architecture that completes tasks at a fraction of the cost when compared to conventional web agents.

Result: 实验或分析结果显示：This is achieved by leveraging a behavioral model of the underlying application constructed by analyzing successful task-execution demonstrations.

Conclusion: 总体来看，论文结论是：Our experiments with 45 tasks on the Canvas Learning Management System (LMS) demonstrate that OdoBot uses 44% and 80% fewer tokens than two state-of-the-art competitor agents (Agent-E and WebVoyager), while also surpassing WebVoyager in terms of task success rate.
...
### [5] [AutoTailor: Automatic, User-Aligned Capability Selection and Adaptation for Web Agents](https://arxiv.org/abs/2609.13548)
...
TL;DR: 本文围绕《AutoTailor: Automatic, User-Aligned Capability Selection and Adaptation for Web Agents》展开，核心内容是：Web agents can utilize reusable tools to reduce the cost and latency of low-level browser interaction, but automatically discovered tool collections can be large, redundant, and poorly aligned with user demand. We present AutoTailor, a meta-agentic framework for constructing and maintaining a compact set of trajectory-derived Model Context Protocol (MCP) APIs.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Online, Dynamic Reselection monitors task outcomes and API usage, identifies recurring coverage gaps, adds relevant candidates, and prunes persistently unused capabilities.

Method: 方法上，论文主要采用或提出了如下思路：We present AutoTailor, a meta-agentic framework for constructing and maintaining a compact set of trajectory-derived Model Context Protocol (MCP) APIs.

Result: 实验或分析结果显示：We evaluate AutoTailor on 106 WebArena Postmill tasks.

Conclusion: 总体来看，论文结论是：This combination improves accuracy and latency while sharply reducing token usage and end-to-end cost, demonstrating the value of user-aligned capability management for efficient web agents.
...
### [6] [LLaDA-UI: Bringing Block-wise Diffusion to Vision-Language GUI Agents](https://arxiv.org/abs/2609.13287)
...
TL;DR: 本文围绕《LLaDA-UI: Bringing Block-wise Diffusion to Vision-Language GUI Agents》展开，核心内容是：Diffusion large language models (dLLMs) achieve high decoding efficiency through block-parallel, arbitrary-order generation, making them attractive for latency-sensitive applications. GUI agents represent a natural testbed for this paradigm, as they must repeatedly perceive screen states and emit structured, spatially grounded actions in real time.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：However, whether dLLMs can be extended into capable multimodal GUI agents while preserving their parallel decoding advantage remains an open question.

Method: 方法上，论文主要采用或提出了如下思路：GUI agents represent a natural testbed for this paradigm, as they must repeatedly perceive screen states and emit structured, spatially grounded actions in real time.

Result: 实验或分析结果显示：Diffusion large language models (dLLMs) achieve high decoding efficiency through block-parallel, arbitrary-order generation, making them attractive for latency-sensitive applications.

Conclusion: 总体来看，论文结论是：These results establish block-wise diffusion as a practical generative paradigm for multimodal GUI agents.
...
### [7] [HazardAuditor: From Executable Threats to Safer Computer-Use Agents](https://arxiv.org/abs/2609.15134)
...
TL;DR: 本文围绕《HazardAuditor: From Executable Threats to Safer Computer-Use Agents》展开，核心内容是：Computer-use agents increasingly interact with browsers, terminals, file systems, and external services, introducing safety risks that emerge through runtime behavior rather than generated content alone. Existing guard models target static prompts and responses and are poorly suited to agent execution; existing executable safety platforms produce evaluation verdicts rather than the normalized supervision a guard model needs to learn across heterogeneous agent frameworks.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：We introduce HazardAuditor, an execution-grounded framework that closes both gaps.

Method: 方法上，论文主要采用或提出了如下思路：Computer-use agents increasingly interact with browsers, terminals, file systems, and external services, introducing safety risks that emerge through runtime behavior rather than generated content alone.

Result: 实验或分析结果显示：Across multiple benchmarks and heterogeneous computer-use systems, HazardAuditor improves accuracy by up to 16.

Conclusion: 总体来看，论文结论是：Code, models, and evaluation artifacts will be available at this https URL .
...
### [8] [Open-UniMo: Towards Unified Motion-Language Understanding and Generation in the Open World](https://arxiv.org/abs/2609.14615)
...
TL;DR: 本文围绕《Open-UniMo: Towards Unified Motion-Language Understanding and Generation in the Open World》展开，核心内容是：Unified motion generation and understanding is crucial for embodied AI systems that can both synthesize and interpret human actions in open-world environments. Existing motion-language models often treat motion as an auxiliary modality of a language model, leading to text-dominated representations and limited cross-modal interaction.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：To address these challenges, we propose Open-UniMo, a unified Large Motion-Language Model (LMLM) trained on million-scale open-world motion-language data.

Method: 方法上，论文主要采用或提出了如下思路：Unified motion generation and understanding is crucial for embodied AI systems that can both synthesize and interpret human actions in open-world environments.

Result: 实验或分析结果显示：Open-UniMo is trained with a two-stage pipeline, where supervised fine-tuning establishes CoT-guided bidirectional motion-language mapping and Group Relative Policy Optimization (GRPO) improves semantic alignment while mitigating cumulative errors in autoregressive motion-token generation.

Conclusion: 总体来看，论文结论是：Furthermore, ablation studies reveal that M2T understanding is not primarily limited by motion-token vocabulary size; instead, coupling M2T with the learnable T2M generation path yields stronger cross-modal representations, demonstrating that generation can facilitate understanding in AR-based motion-language modeling.
...
### [9] [ShieldVLA: Feasibility-Aware Safety Alignment for Vision-Language-Action Models](https://arxiv.org/abs/2609.13231)
...
TL;DR: 本文围绕《ShieldVLA: Feasibility-Aware Safety Alignment for Vision-Language-Action Models》展开，核心内容是：Vision-Language-Action (VLA) models demonstrate strong generalization in robotic manipulation and navigation, but existing fine-tuning methods provide limited safety guarantees. Current approaches primarily rely on Lagrangian optimization that enforces safety through soft penalties on expected cumulative cost, often resulting in residual constraint violations or overly conservative behavior.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Vision-Language-Action (VLA) models demonstrate strong generalization in robotic manipulation and navigation, but existing fine-tuning methods provide limited safety guarantees.

Method: 方法上，论文主要采用或提出了如下思路：We propose ShieldVLA, a safety-aligned fine-tuning framework for VLA models based on Hamilton-Jacobi (HJ) reachability.

Result: 实验或分析结果显示：Current approaches primarily rely on Lagrangian optimization that enforces safety through soft penalties on expected cumulative cost, often resulting in residual constraint violations or overly conservative behavior.

Conclusion: 总体来看，论文结论是：13 over SafeVLA.
...
### [10] [Self-Evolving AI for Humanoids: Mechanisms, Safety, and Evaluation of Post-Deployment Self-Improvement](https://arxiv.org/abs/2609.13236)
...
TL;DR: 本文围绕《Self-Evolving AI for Humanoids: Mechanisms, Safety, and Evaluation of Post-Deployment Self-Improvement》展开，核心内容是：Humanoid robots are becoming an important part of embodied artificial intelligence, driven by advances in reinforcement learning for locomotion, world models for prediction, and vision-language-action models for general control. However, most of these systems remain static after deployment.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：However, most of these systems remain static after deployment.

Method: 方法上，论文主要采用或提出了如下思路：However, most of these systems remain static after deployment.

Result: 实验或分析结果显示：An emerging paradigm of self-evolving agents aims to address this problem by allowing systems to improve from their own post-deployment experience.

Conclusion: 总体来看，论文结论是：Moreover, we outline open challenges spanning AI algorithms, on-board systems, and governance.
...
### [11] [Bridging Thought and Action: Taming Long-Horizon Instability in Open-Source LLM Agents with a MetaTool-Enhanced ROS Framework](https://arxiv.org/abs/2609.13335)
...
TL;DR: 本文围绕《Bridging Thought and Action: Taming Long-Horizon Instability in Open-Source LLM Agents with a MetaTool-Enhanced ROS Framework》展开，核心内容是：Large Language Models (LLMs) have enabled more natural human-robot interaction, but open-source models often exhibit unstable long-horizon reasoning and inefficient action execution when deployed in agentic robotic frameworks. This paper presents an enhanced ROS-Agent based architecture that improves task reliability and execution efficiency for agentic robotic systems using open-source LLMs.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Large Language Models (LLMs) have enabled more natural human-robot interaction, but open-source models often exhibit unstable long-horizon reasoning and inefficient action execution when deployed in agentic robotic frameworks.

Method: 方法上，论文主要采用或提出了如下思路：Large Language Models (LLMs) have enabled more natural human-robot interaction, but open-source models often exhibit unstable long-horizon reasoning and inefficient action execution when deployed in agentic robotic frameworks.

Result: 实验或分析结果显示：This paper presents an enhanced ROS-Agent based architecture that improves task reliability and execution efficiency for agentic robotic systems using open-source LLMs.

Conclusion: 总体来看，论文结论是：Experimental results on real-world interactive tasks demonstrate improved task completion and contextual consistency, with up to ~24% gains on complex tasks compared to the baseline framework.
...
### [12] [STAGE: Diagnosing Semantic Transfer at Grounded Execution in Embodied Agents](https://arxiv.org/abs/2609.13458)
...
TL;DR: 本文围绕《STAGE: Diagnosing Semantic Transfer at Grounded Execution in Embodied Agents》展开，核心内容是：Embodied language grounding requires more than identifying the referent of an instruction: recovered semantics must also control the action an agent exposes. We study this missing link as a semantic-action gap, where instruction semantics are recoverable but weakly expressed in native continuous actions.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：We study this missing link as a semantic-action gap, where instruction semantics are recoverable but weakly expressed in native continuous actions.

Method: 方法上，论文主要采用或提出了如下思路：We introduce SAT-Bench, a fixed-observation counterfactual benchmark that holds the visual scene and agent state fixed while changing only instruction semantics.

Result: 实验或分析结果显示：0% of normal commands, and verified selection further improves target-consistent action exposure without updating the underlying policy.

Conclusion: 总体来看，论文结论是：Overall, embodied language evaluation should measure semantic-action transfer, not semantic parsing alone.
...
### [13] [From Vision to Harvest: Benchmarking Vision-Language Models for Multi-Arm Robotic Fruit Harvesting](https://arxiv.org/abs/2609.13606)
...
TL;DR: 本文围绕《From Vision to Harvest: Benchmarking Vision-Language Models for Multi-Arm Robotic Fruit Harvesting》展开，核心内容是：Multi-arm robotic harvesting offers a promising path to improve harvesting efficiency and reduce reliance on manual labor. However, practical deployment remains challenging because the system must generalize across diverse environments while efficiently coordinating multiple arms in a shared workspace.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：However, practical deployment remains challenging because the system must generalize across diverse environments while efficiently coordinating multiple arms in a shared workspace.

Method: 方法上，论文主要采用或提出了如下思路：However, practical deployment remains challenging because the system must generalize across diverse environments while efficiently coordinating multiple arms in a shared workspace.

Result: 实验或分析结果显示：Multi-arm robotic harvesting offers a promising path to improve harvesting efficiency and reduce reliance on manual labor.

Conclusion: 总体来看，论文结论是：These results highlight both the promise and current limitations of pretrained VLMs for multi-arm robotic harvesting.
...
### [14] [How to Better Train VLAs: Lessons Learned From the REAL-I Challenge at ICRA 2026](https://arxiv.org/abs/2609.13679)
...
TL;DR: 本文围绕《How to Better Train VLAs: Lessons Learned From the REAL-I Challenge at ICRA 2026》展开，核心内容是：How can robot policies learn more effectively from a fixed demonstration budget? The first Real-world Embodied AI Learning (REAL-I) Challenge at ICRA 2026 examined this question through simulation, real-robot evaluation, and an on-site final on a shared dual-arm humanoid platform.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：The first Real-world Embodied AI Learning (REAL-I) Challenge at ICRA 2026 examined this question through simulation, real-robot evaluation, and an on-site final on a shared dual-arm humanoid platform.

Method: 方法上，论文主要采用或提出了如下思路：Their systems combined pretrained vision-language-action models and task-specific imitation policies with different strategies for data curation, staged adaptation, checkpoint selection, and action-space design.

Result: 实验或分析结果显示：We describe the challenge tasks, data and deployment interfaces, and competition results, then compare the approaches contributed by NUS-CLEAR, RCL-Lab, and this http URL .

Conclusion: 总体来看，论文结论是：These observations motivate a view of fixed-data robot learning that integrates data, adaptation, evaluation, and deployment.
...
### [15] [GROOVE: Geometry-Guided Reduction of Operational-Space Jerk in VLA Execution](https://arxiv.org/abs/2609.13695)
...
TL;DR: 本文围绕《GROOVE: Geometry-Guided Reduction of Operational-Space Jerk in VLA Execution》展开，核心内容是：Chunked vision language action (VLA) policies execute several commands per query, but jerk within chunks and across replanning boundaries can induce oscillatory motion and sharp actuator transients. We present GROOVE, an online regulator that searches directional correction regions around the raw three dimensional end effector (EEF) path, without retraining or additional VLA inference.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Chunked vision language action (VLA) policies execute several commands per query, but jerk within chunks and across replanning boundaries can induce oscillatory motion and sharp actuator transients.

Method: 方法上，论文主要采用或提出了如下思路：We present GROOVE, an online regulator that searches directional correction regions around the raw three dimensional end effector (EEF) path, without retraining or additional VLA inference.

Result: 实验或分析结果显示：On a held out LIBERO benchmark, GROOVE achieves the largest reductions among the evaluated methods, reducing translational and rotational EEF jerk by 33.

Conclusion: 总体来看，论文结论是：09%.
...
### [16] [GeomVLA: Unifying Scene, Motion, and Action in 3D](https://arxiv.org/abs/2609.13812)
...
TL;DR: 本文围绕《GeomVLA: Unifying Scene, Motion, and Action in 3D》展开，核心内容是：We present GeomVLA, a Vision-Language-Action (VLA) model that unifies perception, latent scene motion prediction, and action generation within a shared robot-centric 3D coordinate frame. Our approach lifts pretrained VLM features into spatially grounded 3D scene tokens using depth and camera calibration, while retaining the semantic representations learned during VLM pretraining.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：We present GeomVLA, a Vision-Language-Action (VLA) model that unifies perception, latent scene motion prediction, and action generation within a shared robot-centric 3D coordinate frame.

Method: 方法上，论文主要采用或提出了如下思路：We present GeomVLA, a Vision-Language-Action (VLA) model that unifies perception, latent scene motion prediction, and action generation within a shared robot-centric 3D coordinate frame.

Result: 实验或分析结果显示：GeomVLA achieves state-of-the-art performance on CALVIN, competitive performance on LIBERO and RoboTwin2.

Conclusion: 总体来看，论文结论是：Extensive ablations show that future-motion reasoning alone is insufficient: the primary gains are associated with maintaining geometric consistency among scene representation, motion prediction, and robot actions throughout the perception-to-action pipeline.
...
### [17] [ReWeight: Leveraging Human Data for VLA Post-Training via Demonstration Retrieval and Sample Weighting](https://arxiv.org/abs/2609.13851)
...
TL;DR: 本文围绕《ReWeight: Leveraging Human Data for VLA Post-Training via Demonstration Retrieval and Sample Weighting》展开，核心内容是：Post-training vision-language-action (VLA) models for specific robots and tasks requires in-domain demonstrations, yet collecting diverse robot data is costly. Egocentric human demonstrations provide a scalable alternative, but directly mixing human and robot data can introduce cross-embodiment discrepancies and degrade policy performance.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：To address this challenge, we introduce ReWeight, a framework that incorporates human data into VLA post-training through demonstration-level retrieval and sample-level weighting.

Method: 方法上，论文主要采用或提出了如下思路：Egocentric human demonstrations provide a scalable alternative, but directly mixing human and robot data can introduce cross-embodiment discrepancies and degrade policy performance.

Result: 实验或分析结果显示：We evaluate ReWeight using $\pi_{0.

Conclusion: 总体来看，论文结论是：(Project webpage: this https URL )
...
TL;DR: 本文围绕《What Makes an Efficient VLA? Navigating Action-Head Design, Scaling, and Latency》展开，核心内容是：Vision-Language-Action (VLA) models combine a pretrained vision encoder, a language backbone, and an action head, but their relative contribution has not been established under controlled, latency-paired conditions. We fix the backbone families (SigLIP2 and Qwen2.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Third, those returns diminish sharply near the size today's $\pi$-series VLAs already use, so further growth buys little in-domain accuracy for its latency.

Method: 方法上，论文主要采用或提出了如下思路：We read this as representation transfer: the aligned head keeps attending to the instruction's object nouns and stays close to the backbone in weight space rather than relearning to act from scratch.

Result: 实验或分析结果显示：The study yields three findings.

Conclusion: 总体来看，论文结论是：These specify EffVLA, a compact model matching the strongest open-source VLAs on standard LIBERO, leading on most LIBERO-Plus perturbation axes at lower latency, and transferring to a real SO-ARM101 arm with the recipe unchanged.
...
### [19] [Visible Touch: Rendering Contact for Visuomotor Policies](https://arxiv.org/abs/2609.14156)
...
TL;DR: 本文围绕《Visible Touch: Rendering Contact for Visuomotor Policies》展开，核心内容是：Integrating contact information into visuomotor policies remains an open problem. Touch is essential to robust manipulation, yet most modern policies, including pretrained vision-language-action (VLA) models, operate from vision and proprioception alone.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Integrating contact information into visuomotor policies remains an open problem.

Method: 方法上，论文主要采用或提出了如下思路：Existing approaches to closing this gap require specialized tactile hardware, add separate tactile encoders, or commit to non-image policy backbones, all incompatible with the modern paradigm of image-conditioned policies built on pretrained 2D visual representations.

Result: 实验或分析结果显示：Across the LIBERO benchmark, Visible Touch improves BC-Transformer success by 15.

Conclusion: 总体来看，论文结论是：5}$ on four real-world contact-rich tasks gains 30 percentage points with our custom sensor.
...
TL;DR: 本文围绕《Task-Specified Active Metrological Inspection with Measurement-Steered VLA Manipulation and Deterministic Evidence Gating》展开，核心内容是：High-mix low-volume (HMLV) manufacturing requires inspection systems to adapt to changing parts, specifications, and work orders without repeated task-specific programming. Existing inspection automation typically assumes predefined sensing sequences, while general purpose robot agents optimize task completion rather than the completeness and validity of metrological evidence.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：High-mix low-volume (HMLV) manufacturing requires inspection systems to adapt to changing parts, specifications, and work orders without repeated task-specific programming.

Method: 方法上，论文主要采用或提出了如下思路：High-mix low-volume (HMLV) manufacturing requires inspection systems to adapt to changing parts, specifications, and work orders without repeated task-specific programming.

Result: 实验或分析结果显示：A series of physical experiments shows that FRAME achieves higher end-to-end inspection reliability, fewer false accepts, and shorter task completion time.

Conclusion: 总体来看，论文结论是：A series of physical experiments shows that FRAME achieves higher end-to-end inspection reliability, fewer false accepts, and shorter task completion time.
...
TL;DR: 本文围绕《Multi-Task Visual Perception Network with LLM Conditioning for Autonomous Navigation》展开，核心内容是：Long-term navigation for service robots faces crit- ical challenges like the accumulation of odometry drift and sensor error, which progressively degrade 2D maps and renders traditional path planning algorithms (e. g.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Long-term navigation for service robots faces crit- ical challenges like the accumulation of odometry drift and sensor error, which progressively degrade 2D maps and renders traditional path planning algorithms (e.

Method: 方法上，论文主要采用或提出了如下思路：To address this, we propose a user-friendly, interactive framework that eliminates the reliance on globally consistent maps.

Result: 实验或分析结果显示：Experiments on real-world and simulated data have shown significant improve- ments over other methods.

Conclusion: 总体来看，论文结论是：Our source code is publicly accessible at this https URL .
...
### [22] [EMoG: Emotion-Modulated Gait Generation for Expressive Humanoid Locomotion](https://arxiv.org/abs/2609.14432)
...
TL;DR: 本文围绕《EMoG: Emotion-Modulated Gait Generation for Expressive Humanoid Locomotion》展开，核心内容是：Existing humanoid locomotion systems primarily focus on stability and task execution, while integrating expressiveness with explicit locomotion control remains challenging. We propose EMoG, an emotion-modulated gait generation framework for expressive humanoid locomotion.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Existing humanoid locomotion systems primarily focus on stability and task execution, while integrating expressiveness with explicit locomotion control remains challenging.

Method: 方法上，论文主要采用或提出了如下思路：Existing humanoid locomotion systems primarily focus on stability and task execution, while integrating expressiveness with explicit locomotion control remains challenging.

Result: 实验或分析结果显示：Experiments demonstrate continuous gait-style modulation with perceptible expressive cues while maintaining command tracking.

Conclusion: 总体来看，论文结论是：EMoG provides a practical approach to parameterized emotional-style walking for human-robot interaction.
...
### [23] [NavPatch: Evidence-Guided Object-Level Costmap Correction with Vision-Language Models](https://arxiv.org/abs/2609.14543)
...
TL;DR: 本文围绕《NavPatch: Evidence-Guided Object-Level Costmap Correction with Vision-Language Models》展开，核心内容是：Mobile robots typically rely on geometric maps for obstacle avoidance and path planning, but the resulting obstacle representation does not always match how an object should affect navigation. A low lying cable may be missed, a flexible curtain may create spurious blockage, and a traffic cone may require an exclusion region larger than its observed footprint.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Mobile robots typically rely on geometric maps for obstacle avoidance and path planning, but the resulting obstacle representation does not always match how an object should affect navigation.

Method: 方法上，论文主要采用或提出了如下思路：Mobile robots typically rely on geometric maps for obstacle avoidance and path planning, but the resulting obstacle representation does not always match how an object should affect navigation.

Result: 实验或分析结果显示：Mobile robots typically rely on geometric maps for obstacle avoidance and path planning, but the resulting obstacle representation does not always match how an object should affect navigation.

Conclusion: 总体来看，论文结论是：7% compared with updates based only on the current observation.
...
### [24] [GLAM: Training a latent world model over global spatiotemporal memory for active exploration and navigation](https://arxiv.org/abs/2609.14561)
...
TL;DR: 本文围绕《GLAM: Training a latent world model over global spatiotemporal memory for active exploration and navigation》展开，核心内容是：Active exploration and semantic navigation require an embodied agent to build memory from partial observations, predict how the evolution of observed spatial memory may support future motion, and convert that prediction into actionable plans. We present GLAM, a goal-conditioned latent world model trained over global spatiotemporal memory, and GLAM NAV, the complete navigation system built around it.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Active exploration and semantic navigation require an embodied agent to build memory from partial observations, predict how the evolution of observed spatial memory may support future motion, and convert that prediction into actionable plans.

Method: 方法上，论文主要采用或提出了如下思路：We present GLAM, a goal-conditioned latent world model trained over global spatiotemporal memory, and GLAM NAV, the complete navigation system built around it.

Result: 实验或分析结果显示：On a controlled HM3D-ObjectNav subset reproduction setting, GLAM NAV improves over a reproduced BSC-Nav baseline in both success rate and success weighted by path length.

Conclusion: 总体来看，论文结论是：On a controlled HM3D-ObjectNav subset reproduction setting, GLAM NAV improves over a reproduced BSC-Nav baseline in both success rate and success weighted by path length.
...
### [25] [IMPACT-VLA: Interaction-aware Multimodal Propagation Attribution via Counterfactual Trajectories for Vision-Language-Action Policies](https://arxiv.org/abs/2609.15005)
...
TL;DR: 本文围绕《IMPACT-VLA: Interaction-aware Multimodal Propagation Attribution via Counterfactual Trajectories for Vision-Language-Action Policies》展开，核心内容是：Vision-Language-Action (VLA) policies perform robot manipulation tasks using multimodal inputs such as visual observations, proprioceptive states, and language instructions. However, it remains unclear at which execution stages each modality contributes to final task success and how input interventions propagate through subsequent states, observations, and actions.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：However, it remains unclear at which execution stages each modality contributes to final task success and how input interventions propagate through subsequent states, observations, and actions.

Method: 方法上，论文主要采用或提出了如下思路：We propose Interaction-aware Multimodal Propagation Attribution via Counterfactual Trajectories for Vision-Language-Action Policies (IMPACT-VLA).

Result: 实验或分析结果显示：These results reveal when multimodal inputs support task success and how their contributions become conditionally coupled during closed-loop execution.

Conclusion: 总体来看，论文结论是：These results reveal when multimodal inputs support task success and how their contributions become conditionally coupled during closed-loop execution.
...
### [26] [Atomic Motion Coordinate for Language-Steerable and Force-Responsive Manipulation](https://arxiv.org/abs/2609.15012)
...
TL;DR: 本文围绕《Atomic Motion Coordinate for Language-Steerable and Force-Responsive Manipulation》展开，核心内容是：Can changing only the language instruction redirect a VLA policy's end effector, or does the visually driven motion prior dominate? We present Atomic Motion Coordinate, a geometry-grounded coordinate for steerable and force-responsive manipulation.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Can changing only the language instruction redirect a VLA policy's end effector, or does the visually driven motion prior dominate?

Method: 方法上，论文主要采用或提出了如下思路：We present Atomic Motion Coordinate, a geometry-grounded coordinate for steerable and force-responsive manipulation.

Result: 实验或分析结果显示：Each arm owns thirteen signed translation, rotation, and hold atoms grounded from text and forward kinematics with vision withheld, and the coordinate is injected into every action-expert block via weighted codebook alignment.

Conclusion: 总体来看，论文结论是：2%.
...
### [27] [C$^2$Nav: Compare Before You Commit for Zero-Shot Vision-and-Language Navigation](https://arxiv.org/abs/2609.15142)
...
TL;DR: 本文围绕《C$^2$Nav: Compare Before You Commit for Zero-Shot Vision-and-Language Navigation》展开，核心内容是：Zero-shot vision-and-language navigation in continuous environments (VLN-CE) increasingly places foundation vision-language models (VLMs) inside the navigation loop. Existing systems commonly request cardinal outputs such as waypoints, pixels, headings, progress values, or absolute arrival decisions, coupling a generative response to geometric magnitude or an irreversible commitment.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：We study a complementary model-robot interface: the VLM compares controller-constructed alternatives, while geometry, thresholds, action magnitude, and execution remain on the physical side.

Method: 方法上，论文主要采用或提出了如下思路：Existing systems commonly request cardinal outputs such as waypoints, pixels, headings, progress values, or absolute arrival decisions, coupling a generative response to geometric magnitude or an irreversible commitment.

Result: 实验或分析结果显示：The results indicate that a constrained decision interface and stronger VLM reasoning are complementary rather than interchangeable.

Conclusion: 总体来看，论文结论是：The results indicate that a constrained decision interface and stronger VLM reasoning are complementary rather than interchangeable.
...
### [28] [HarnessVLN: Unifying Training-Free Embodied Navigation through an Agent Harness](https://arxiv.org/abs/2609.15195)
...
TL;DR: 本文围绕《HarnessVLN: Unifying Training-Free Embodied Navigation through an Agent Harness》展开，核心内容是：Embodied navigation requires agents to interpret visual observations, accumulate spatial knowledge, and execute actions to follow instructions or locate objects. Training-based methods face generalization challenges, while training-free methods exploit multimodal large language models (MLLMs) but often lack mechanisms to reconcile proposed actions with spatial evidence, task progress, and execution failures.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Training-based methods face generalization challenges, while training-free methods exploit multimodal large language models (MLLMs) but often lack mechanisms to reconcile proposed actions with spatial evidence, task progress, and execution failures.

Method: 方法上，论文主要采用或提出了如下思路：Training-based methods face generalization challenges, while training-free methods exploit multimodal large language models (MLLMs) but often lack mechanisms to reconcile proposed actions with spatial evidence, task progress, and execution failures.

Result: 实验或分析结果显示：HarnessVLN achieves success rates of 60.
...
### [29] [InterSocialBench: Benchmarking Human and LLM Preferences for Companion-Robot Social Behavior](https://arxiv.org/abs/2609.15455)
...
TL;DR: 本文围绕《InterSocialBench: Benchmarking Human and LLM Preferences for Companion-Robot Social Behavior》展开，核心内容是：Companion robots face everyday situations in which several feasible behaviors may be appropriate, yet different people prefer different responses. We introduce InterSocialBench, a benchmark of 210 domestic scenarios and 18 high-level behaviors, pairing judgments from 100 human participants with 23,520 responses from seven large language models under 16 personality conditions.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Across the tested prompts, model and human behavior distributions differ, and the diversity gap remains after matching response counts: humans exhibit 4.

Method: 方法上，论文主要采用或提出了如下思路：We introduce InterSocialBench, a benchmark of 210 domestic scenarios and 18 high-level behaviors, pairing judgments from 100 human participants with 23,520 responses from seven large language models under 16 personality conditions.

Result: 实验或分析结果显示：Each human annotation preserves a preferred action alongside explicitly appropriate and inappropriate candidates.

Conclusion: 总体来看，论文结论是：InterSocialBench supports evaluating social behavior selection without replacing individual judgments with a single consensus label.
...
### [30] [WLA$^3$: World Latent Action Modeling for Semantics, Dynamics, and Kinematics](https://arxiv.org/abs/2609.15870)
...
TL;DR: 本文围绕《WLA$^3$: World Latent Action Modeling for Semantics, Dynamics, and Kinematics》展开，核心内容是：Scaling generalist policy models with heterogeneous data is limited by the lack of unified, low-noise action supervision. Human egocentric videos are abundant, but only a small fraction comes with high-quality hand-action labels.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Scaling generalist policy models with heterogeneous data is limited by the lack of unified, low-noise action supervision.

Method: 方法上，论文主要采用或提出了如下思路：We introduce WLA$^3$ (World Latent Action Modeling for Semantics, Dynamics, and Kinematics), a unified generalist policy model framework built around representations learned by a World Latent Action Model (WLAM).

Result: 实验或分析结果显示：WLA$^3$ achieves 81.

Conclusion: 总体来看，论文结论是：Project page can be found at this https URL .
...
### [31] [Beyond Single-Axis Testing: Paired Evaluation of Compound Robustness in Vision-Language-Action Policies](https://arxiv.org/abs/2609.15940)
...
TL;DR: 本文围绕《Beyond Single-Axis Testing: Paired Evaluation of Compound Robustness in Vision-Language-Action Policies》展开，核心内容是：Vision-language-action policies are typically evaluated one perturbation at a time, providing a useful diagnosis of their sensitivity to individual distribution shifts. Real-world deployment, however, may involve several shifts simultaneously, and it remains unclear how these individual robustness measurements compose.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Real-world deployment, however, may involve several shifts simultaneously, and it remains unclear how these individual robustness measurements compose.

Method: 方法上，论文主要采用或提出了如下思路：We introduce LIBERO-CTRL, a six-axis benchmark that pairs each initial state across single-axis conditions and a matched simultaneous condition.

Result: 实验或分析结果显示：Vision-language-action policies are typically evaluated one perturbation at a time, providing a useful diagnosis of their sensitivity to individual distribution shifts.

Conclusion: 总体来看，论文结论是：Compound robustness therefore cannot be characterized from aggregate single-axis success rates alone; matched per-instance evaluation is needed to reveal how joint perturbations alter behavior.
...
TL;DR: 本文围绕《MessyMem: Learning-from-Doing Memory for Mobile Manipulation》展开，核心内容是：Mobile manipulators deployed across many rooms and visits should improve with experience: after discovering that a cabinet is locked or finding an object in a drawer, the robot should reuse that knowledge rather than start each task from scratch. Yet today's robots often treat each task as new: compact scene representations omit interaction-derived knowledge, raw video histories are difficult to query, and VLM planners reason at inference time without persistently updating what the robot knows.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Mobile manipulators deployed across many rooms and visits should improve with experience: after discovering that a cabinet is locked or finding an object in a drawer, the robot should reuse that knowledge rather than start each task from scratch.

Method: 方法上，论文主要采用或提出了如下思路：Yet today's robots often treat each task as new: compact scene representations omit interaction-derived knowledge, raw video histories are difficult to query, and VLM planners reason at inference time without persistently updating what the robot knows.

Result: 实验或分析结果显示：Mobile manipulators deployed across many rooms and visits should improve with experience: after discovering that a cabinet is locked or finding an object in a drawer, the robot should reuse that knowledge rather than start each task from scratch.

Conclusion: 总体来看，论文结论是：9 points, while retrieving task-relevant evidence from thousands of stored keyframes and over an hour into the past.
...
TL;DR: 本文围绕《PhysBrain 1.5: From Vision-Language Models to Physical Foundation Models》展开，核心内容是：We present PhysBrain 1. 5, a unified model for understanding physical environments, generating actions, and predicting future states.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：We present PhysBrain 1.

Method: 方法上，论文主要采用或提出了如下思路：We present PhysBrain 1.

Result: 实验或分析结果显示：Across 28 embodied understanding benchmarks, our 8B model achieves an average score of 72.

Conclusion: 总体来看，论文结论是：Beyond these understanding evaluations, qualitative examples show the model's ability to produce end-effector trajectories and predict future scenes through spatially aligned RGB, depth, and robot-mask outputs.
...
### [34] [GRAVA: Grounded Reasoning-to-Action Representation and Learning for Autonomous Driving](https://arxiv.org/abs/2609.15169)
...
TL;DR: 本文围绕《GRAVA: Grounded Reasoning-to-Action Representation and Learning for Autonomous Driving》展开，核心内容是：Driving vision-language-action (VLA) models increasingly reason before acting, but their intermediate reasoning is often weakly grounded in physical scene evidence and loosely connected to executable behavior. We present GRAVA, a framework built around Grounded Reasoning-to-Action (GRA), which unifies grounding, reasoning, and action generation in a single autoregressive stream.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Driving vision-language-action (VLA) models increasingly reason before acting, but their intermediate reasoning is often weakly grounded in physical scene evidence and loosely connected to executable behavior.

Method: 方法上，论文主要采用或提出了如下思路：We present GRAVA, a framework built around Grounded Reasoning-to-Action (GRA), which unifies grounding, reasoning, and action generation in a single autoregressive stream.

Result: 实验或分析结果显示：A progressive training strategy develops grounded cognition through pre-training, establishes the reasoning-to-action interface through imitation, and improves driving behavior through reinforcement learning and exploration.

Conclusion: 总体来看，论文结论是：These results show the benefit of preserving action-relevant physical evidence from grounded reasoning through executable action generation.
...
### [35] [PhysMent: An Interactive Approach For LLM Reasoning In Physics Problems](https://arxiv.org/abs/2609.13152)
...
TL;DR: 本文围绕《PhysMent: An Interactive Approach For LLM Reasoning In Physics Problems》展开，核心内容是：Large language models (LLMs) perform strongly on static science benchmarks, yet their ability to reason about the physical world through active experimentation remains poorly understood. We introduce PhysMent, a benchmark that evaluates LLM physical reasoning via iterative, toolmediated interaction with a MuJoCo physics simulator.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Large language models (LLMs) perform strongly on static science benchmarks, yet their ability to reason about the physical world through active experimentation remains poorly understood.

Method: 方法上，论文主要采用或提出了如下思路：Large language models (LLMs) perform strongly on static science benchmarks, yet their ability to reason about the physical world through active experimentation remains poorly understood.

Result: 实验或分析结果显示：Large language models (LLMs) perform strongly on static science benchmarks, yet their ability to reason about the physical world through active experimentation remains poorly understood.

Conclusion: 总体来看，论文结论是：Across the seven models, accuracy ranges from 25% to 67%, with failures due to premature answer submission, inefficient exploration, and inconsistent grounding in simulator feedback rather than conceptual gaps.
...
### [36] [TATK: Triple-Aware Top-K Learning with Knowledge-Grounded Verification for LLM-based Sequential Recommendation](https://arxiv.org/abs/2609.14565)
...
TL;DR: 本文围绕《TATK: Triple-Aware Top-K Learning with Knowledge-Grounded Verification for LLM-based Sequential Recommendation》展开，核心内容是：LLM-based sequential recommenders usually cast next-item prediction as text generation, but this interface is poorly matched to full-catalog top-K ranking. We propose TATK, a Triple-Aware framework that couples Top-K Learning (TKL) with Knowledge-Grounded Verification (KGV) for LLM-based sequential recommendation.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：LLM-based sequential recommenders usually cast next-item prediction as text generation, but this interface is poorly matched to full-catalog top-K ranking.

Method: 方法上，论文主要采用或提出了如下思路：We propose TATK, a Triple-Aware framework that couples Top-K Learning (TKL) with Knowledge-Grounded Verification (KGV) for LLM-based sequential recommendation.

Result: 实验或分析结果显示：We evaluate TATK on Musical Instruments, CDs and Vinyl, and Video Games from Amazon Reviews 2023 under a matched R2ec-style full-catalog protocol.

Conclusion: 总体来看，论文结论是：The diagnostics show that structural evidence is most useful for recoverable top-M candidates with reliable KG support, and should be gated when metadata relations are sparse or noisy.
...
### [37] [Rethinking the Implications of Human Feedback for Preference Learning in Human-Robot Collaboration](https://arxiv.org/abs/2609.13982)
...
TL;DR: 本文围绕《Rethinking the Implications of Human Feedback for Preference Learning in Human-Robot Collaboration》展开，核心内容是：In Human-Robot Interaction, the standard approach to learn a reward model that represents human preferences for robot behavior consists of three steps. First, the robot collects limited direct evidence from human feedback (e.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：In Human-Robot Interaction, the standard approach to learn a reward model that represents human preferences for robot behavior consists of three steps.

Method: 方法上，论文主要采用或提出了如下思路：In Human-Robot Interaction, the standard approach to learn a reward model that represents human preferences for robot behavior consists of three steps.

Result: 实验或分析结果显示：Unfortunately, the fixed rule can hinder preference learning: in a user study with two collaborative simulation environments, human-provided implication labels often differed from the standard fixed rule, and using the human labels substantially improved reward learning with the Preference Learning from Implicit and Explicit Feedback (PIE) algorithm.

Conclusion: 总体来看，论文结论是：By learning to reason about the implications of human feedback, this work enables more faithful and efficient robot behavior adaptation during human-robot collaboration.
...
### [38] [Part Grounding, Not Action Knowledge: Locating the Bottleneck in VLM Affordance Prediction](https://arxiv.org/abs/2609.13225)
*Sarthak Sattigeri*
...
TL;DR: 本文围绕《Part Grounding, Not Action Knowledge: Locating the Bottleneck in VLM Affordance Prediction》展开，核心内容是：Benchmarks agree that vision-language models reason poorly about low-level manipulation, but an aggregate accuracy score does not say which step fails. We separate two steps that affordance questions conflate: identifying which part of an object to act on, and knowing what action that part requires.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：Benchmarks agree that vision-language models reason poorly about low-level manipulation, but an aggregate accuracy score does not say which step fails.

Method: 方法上，论文主要采用或提出了如下思路：Benchmarks agree that vision-language models reason poorly about low-level manipulation, but an aggregate accuracy score does not say which step fails.

Result: 实验或分析结果显示：Inspecting the outputs showed why: models described a different part than the one being scored, e.

Conclusion: 总体来看，论文结论是：929 and a labelling rule wrong on 4 of 19 objects, both caught only by testing our numbers against trivial alternatives.
...
### [39] [When Faster VLA Deployment Changes Closed-Loop Behavior: Task Success-Latency Analysis of SmolVLA Across PyTorch and ONNX Variants](https://arxiv.org/abs/2609.14146)
*Rafiqul Islam*
...
TL;DR: 本文围绕《When Faster VLA Deployment Changes Closed-Loop Behavior: Task Success-Latency Analysis of SmolVLA Across PyTorch and ONNX Variants》展开，核心内容是：Vision-language-action (VLA) deployment can reduce inference latency while changing closed-loop task behavior. We evaluate HuggingFaceVLA/smolvla_libero on an RTX 2060 (6 GB) in LIBERO Spatial and Object (MuJoCo 3.


<details>
  <summary>Details</summary>
Motivation: 研究动机来自现有方法或系统中的不足：0% and Object remains at 89.

Method: 方法上，论文主要采用或提出了如下思路：We evaluate HuggingFaceVLA/smolvla_libero on an RTX 2060 (6 GB) in LIBERO Spatial and Object (MuJoCo 3.

Result: 实验或分析结果显示：We evaluate HuggingFaceVLA/smolvla_libero on an RTX 2060 (6 GB) in LIBERO Spatial and Object (MuJoCo 3.
